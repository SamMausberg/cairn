...
A borrow argument names a place: something that holds a value and can be assigned or lent, such as a local (`frame`), a field (`f.body`) or an element (`xs[i]`). An `ro<T>` parameter also accepts a temporary, such as `1 + 2`. An `rw` parameter and a view of an array need a place, or a string literal for an `ro<u8>[n]` (`E-CALL-VIEW`). A view is not one value, so passing `xs` where an `ro<T>` or `rw<T>` is expected is `E-TYPE-MISMATCH`: pass an element, `xs[i]`.

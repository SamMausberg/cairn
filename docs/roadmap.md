...
A demanding runtime needs four things from the language: bounded execution, resources it can reuse, ownership that holds across asynchronous work, and failure it handles explicitly. Task groups bound the tasks in flight and keep what they borrow leased across the asynchronous part. Four gates stay open: cancellation, asynchronous I/O without a thread per task (a completion queue over the kernel; io_uring is enabled on the reference host), recovery from the loss of a device, and collectives across devices. The last two cannot be exercised on the reference machine, which has one GPU and runs device code only under `make gpu`.

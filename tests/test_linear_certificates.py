...
import pytest
from cairn.linear_certificates import Rule,Certificate,check,collector_rules,audit_collector
...
def test_compiler_refuses_failed_certificate(monkeypatch):
    ...
    with pytest.raises(ValueError): compile_source('fn id(x:u64)->u64=x;')
...
def test_independent_finite_sanity():
    ...
    for rule,cert in collector_rules():
        for k,i,n,m in product(range(5),repeat=4):
            ...
            if all(eval_form(f)>=0 for f in rule.assumptions): assert eval_form(rule.conclusion)>=0

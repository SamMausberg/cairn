...
Every pull request carries a short description a reviewer can check in a minute: what changes and why, how it was checked (the commands and what they reported), and what a reviewer should look at, such as a rule it touches, generated code it changes or evidence it adds. Three to six lines, plain sentences, no headings beyond the template's, no emojis. Each commit on the branch gets the same care in one line.

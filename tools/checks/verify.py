...
def main():
    ...
    def run(argv, output=None, env=None):
        ...
        (ROOT / "results/verification_run.json").write_text(json.dumps(record, indent=2) + "\n")
        if cp.returncode:
    ...
    for stem in ["native", "family", "wire"]:
        run([py, "tools/release/build.py", f"examples/basics/{stem}.cairn", "--out", "results", "--arch", arch])
    run(command("clang++", "bench/cpu/family_template.cpp", "results/libtemplate.so", arch, "library"))
    # The suite's own smoke test runs this script; the flag stops it re-entering and rewriting results/.
    run([py, "-m", "pytest", a.tests, "-q"], "results/compiler_tests.txt", dict(os.environ, CAIRN_VERIFY="1"))
    run([py, "tests/checks/native_checks.py"], "results/native_tests.json")
    run([py, "tests/checks/collector_wire_checks.py"], "results/collector_wire_tests.json")
    run([py, "tests/checks/template_checks.py"], "results/template_tests.json")
    if a.gcc:
        for stem in ["native", "family"]:
            run(command("g++", f"results/{stem}.cpp", f"results/lib{stem}_gcc.so", arch, "library"))
        env = dict(os.environ, CAIRN_FAMILY_LIB=str(ROOT / "results/libfamily_gcc.so"))
        run(
            [py, "tests/checks/native_checks.py", str(ROOT / "results/libnative_gcc.so")],
    ...
    if a.sanitize:
        ...
        run(["clang++", *flags, "tests/native/sanitize.cpp", "-o", "results/sanitize"])
    ...
    if a.bench:
        run([py, "bench/cpu/run.py"], "results/benchmark_run.txt")
    run([py, "tools/checks/density.py"], "results/density_run.txt")

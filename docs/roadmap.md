...
**Language.** Typed recipes written by library authors (the general form of `family`, `derive wire` and the collector) are not implemented; the three closed generators remain. Owned dynamic values (a boxed `dyn`), asynchronous I/O with completion tickets, multi-device collectives and a checked integer reduction are absent. Separate compilation exists as one translation unit per program; per-module objects with interface digests are the next build step. Generic code is checked per instance, not once against its bounds.

...
This demo shows one numerical kernel written once and run two ways: on the host's lane pool and as CUDA lanes on a GPU. A square plate of 1024 x 1024 cells has its top edge held at 100 degrees and its other edges at 0. Jacobi relaxation finds its temperature: each sweep sets every interior cell to the mean of its four neighbours.

The sweep is written once, as a `parallel` region, a loop whose iterations CAIRN may run at once. With host views it runs on the lane pool, and with `@device` views the same body runs as CUDA lanes.
...
`src/plate.cairn` holds the plate. The arithmetic of one cell is a pure function, which host lanes and device lanes may both call:

...
class Checker:
    ...
    def type_ok(self, ty: Type, node=None):
        if (
            ty.name not in CPP
            and ty.name not in self.p.records
            and ty.name not in self.p.enums
            and ty.name not in self.p.sums
        ):
            fail("E-TYPE", f"Unknown type {ty.name}.", node)
        if ty.mode != "value" and ty.name == "void":
            fail("E-TYPE", "A slice cannot contain void.", node)
        if ty.mode != "value" and ty.name in self.p.sums:
            fail(
                "E-SUM-VIEW",
                "Borrowed arrays of tagged payloads are not in this ABI profile.",
                node,
            )
    ...
    def expect(self, got: Type, want: Type, e: Expr):
        if got != want:
            fail(
                "E-TYPE-MISMATCH",
                f"Expected {want.display()}, got {got.display()}.",
                e,
                expected_type=want.display(),
                actual_type=got.display(),
            )
    ...
    def check(self) -> dict[str, Any]:
        ...
        for n, fields in self.p.records.items():
            if n in CPP:
                fail("E-TYPE-NAME", f"Cannot redefine builtin {n}.")
            if not fields:
                fail("E-RECORD", "Empty records are outside this ABI subset.")
            if len({x for x, _ in fields}) != len(fields):
                fail("E-DUPLICATE", f"Duplicate field in {n}.")
            for _, ty in fields:
                if ty.mode != "value" or ty.name not in seen or ty.name == "void":
                    fail(
                        "E-RECORD-TYPE",
                        "Record fields must be previously declared value types, not views or void.",
                    )
            seen.add(n)
        for n, variants in self.p.enums.items():
            if n in CPP or not variants or len(set(variants)) != len(variants):
                fail("E-ENUM", f"Invalid enum {n}.")
        for n, variants in self.p.sums.items():
            ...
            for v, ty in variants:
                if ty is not None and (ty.mode != "value" or ty.name not in NUMERIC | {"bool"}):
                    fail(
                        "E-SUM-PAYLOAD",
                        "Payloads currently require scalar value types, never borrows.",
                    )
        for f in self.p.functions:
            ...
            for n, ty in f.params:
                self.type_ok(ty, f)
                if n in self.env or ty.name == "void":
                ...
                if ty.mode != "value":
                    if ty.extent.isdigit():
                        ...
                        fail(
                            "E-EXTENT",
                            "Dynamic extent must name an earlier immutable usize parameter.",
                            f,
                        )
            ...
            if f.ret.name != "void" and not terminated:
        ...
        self.local_effects = {
            f: {exposed(f, e) for e in es} for f, es in self.local_effects.items()
        }
    ...
    def stmt(self, s: Stmt) -> bool:
        if s.tag in {"buffer", "stack"}:
            ...
            if extent.tag not in {"name", "int"}:
                fail(
                    "E-OWNER-EXTENT",
                    "Bind a computed capacity to an immutable usize first.",
                    extent,
                )
            ...
            if s.tag == "stack":
                ...
                size = (
                    1
                    if s.ty.name == "bool"
                    else WIDTH.get(s.ty.name, 32 if s.ty.name == "f32" else 64) // 8
                )
                prior = sum(
                    x.get("bytes", 0) for x in self.resources[self.f.name] if x["kind"] == "stack"
                )
                if prior + int(extent.val) * size > 65536:
                    fail(
                        "E-STACK-LIMIT",
                        "Explicit stack declarations total at most 65536 bytes per function.",
                        extent,
                    )
            ...
            self.resources[self.f.name].append(
                {
                    "name": s.name,
                    "kind": s.tag,
                    "element": s.ty.name,
                    ...
                    "release": "lexical-on-normal-exit",
                    "line": s.line,
                }
            )
            if s.tag == "stack":
                self.resources[self.f.name][-1]["bytes"] = int(extent.val) * size
            ...
            if s.name in self.env:
                fail(
                    "E-SHADOW",
                    f"{s.name} is already bound; shadowing is forbidden in this subset.",
                    s,
                )
            if s.ty:
                self.type_ok(s.ty, s)
            ty = self.expr(s.exprs[0], s.ty)
            if ty.mode != "value" or ty.name == "void":
                fail(
                    "E-VIEW-ALIAS", "Local view aliases and void values are outside this subset.", s
                )
            s.ty = ty
            self.env[s.name] = Binding(ty, s.tag == "reg")
        elif s.tag == "compact":
            ...
            self.expr(out)
            self.expr(hi, Type("usize"))
            known_extent = hi.val if hi.tag in {"name", "int"} else None
            if (
                hi.tag == "call"
                and hi.val == "len"
                and len(hi.args) == 1
                and hi.args[0].tag == "name"
            ):
                known_extent = self.env[hi.args[0].val].ty.extent
            if known_extent != t.extent:
                fail(
                    "E-COLLECT-CAPACITY",
                    "Compaction requires iteration extent equal to output capacity.",
                    hi,
                )
            self.env[s.binder] = Binding(Type("usize"))
            self.expr(pred, Type("bool"))
            self.expr(value, Type(t.name))
            ...
            if mentions(pred, out.val) or mentions(value, out.val):
                fail(
                    "E-COLLECT-SELF-READ",
                    "Collector predicate/projection cannot read its output.",
                    s,
                )
            ...
            variants = (
                dict(self.p.sums[ty.name])
                if ty.name in self.p.sums
                else dict.fromkeys(self.p.enums[ty.name])
            )
            ...
            if set(given) != expected:
                fail(
                    ...
                    s,
                    missing_variants=sorted(expected - set(given)),
                    unknown_variants=sorted(set(given) - expected),
                )
            returns = []
            for arm in s.arms:
                ...
                if bool(arm.binder) != (payload is not None):
                    fail(
                        "E-MATCH-BINDING",
                        "A payload arm binds exactly one value; a nullary arm binds none.",
                        arm,
                    )
                if arm.binder:
                    if arm.binder in self.env:
                        fail("E-SHADOW", "Payload binder must be fresh.", arm)
            ...
            self.expr(s.exprs[0], Type("usize"))
            self.expr(s.exprs[1], Type("usize"))
            if s.name in self.env:
                fail("E-SHADOW", f"Loop binder {s.name} already exists.", s)
            self.env[s.name] = Binding(Type("usize"))
            self.loop_depth += 1
            self.block(s.body)
            self.loop_depth -= 1
    ...
    def index(self, e: Expr, read: bool = True) -> Type:
        ...
        self.expr(i, Type("usize"))
        self.guard("bounds")
        if read:
            self.effect("read:" + a.val)
        length = ty.extent if ty.extent.isdigit() else "v_" + ty.extent
        e.ty = Type(ty.name)
        e.cpp = f"cr::at({a.cpp}, {i.cpp}, {length})"
        return e.ty
    ...
    def expr(self, e: Expr, expected: Type | None = None) -> Type:
        ...
        if tag == "int":
            ...
            if ty.name in WIDTH:
                ...
                fail(
                    "E-LITERAL-RANGE",
                    "Large integer-to-float literals require a checked explicit conversion.",
                    e,
                )
            ...
            if e.val not in self.env:
                fail(
                    "E-UNBOUND",
                    f"Unbound name {e.val}.",
                    e,
                    available_names=sorted(self.env),
                    expected_type=expected.display() if expected else None,
                )
            b = self.env[e.val]
            ty = b.ty
            e.cpp = (
                f"static_cast<std::size_t>({b.constant}ULL)"
                if b.constant is not None
                else "v_" + e.val
            )
        elif tag == "index":
            ty = self.index(e)
            ...
            if a.tag == "name" and a.val in self.p.sums and a.val not in self.env:
                ...
                if e.val not in self.p.enums[a.val]:
                    fail("E-ENUM-VARIANT", f"Unknown {a.val}.{e.val}.", e)
                ty = Type(a.val)
                e.cpp = "ct_" + a.val + "::v_" + e.val
            else:
                at = self.expr(a)
                if at.mode != "value" or at.name not in self.p.records:
                    fail("E-FIELD", "Field access requires a record.", e)
                fs = dict(self.p.records[at.name])
                if e.val not in fs:
                    fail("E-FIELD", f"Unknown field {e.val}.", e)
                ty = fs[e.val]
                e.cpp = f"({a.cpp}).v_{e.val}"
        elif tag == "unary":
            a = e.args[0]
            ty = self.expr(a, Type("bool") if e.val == "!" else expected)
            if ty.mode != "value":
                fail("E-OPERATOR", "Unary operator on a view.", e)
            if e.val == "!":
                self.expect(ty, Type("bool"), e)
                e.cpp = f"(!{a.cpp})"
            elif e.val == "~":
            ...
            elif ty.name in FLOAT:
                e.cpp = f"(-{a.cpp})"
            elif ty.name in SIGNED:
                self.guard("overflow")
                e.cpp = f"cr::sub<{ty.cpp()}>(0, {a.cpp})"
            else:
            ...
            if logical:
                self.expect(left, Type("bool"), e)
                ty = left
                e.cpp = f"({a.cpp} {op} {b.cpp})"
            elif compare:
                if left.name not in NUMERIC | {"bool"} and not (
                    op in {"==", "!="} and left.name in self.p.enums
                ):
                ...
                ty = left
                e.cpp = f"static_cast<{ty.cpp()}>({a.cpp} {op} {b.cpp})"
            elif left.name in FLOAT and op != "%":
                ty = left
                e.cpp = f"({a.cpp} {op} {b.cpp})"
            elif left.name in INT:
                ty = left
                fn = {"+": "add", "-": "sub", "*": "mul", "/": "divide", "%": "remainder"}[op]
            ...
            fail("E-INTERNAL", f"Unknown expression {tag}.", e)
        e.ty = ty
        if expected:
        ...
        if self.capture_sites and e.start >= 0:
            self.sites.append(
                {
                    "symbol": self.f.name if self.f else "",
                    "start": e.start,
                    "end": e.end,
                    "tag": e.tag,
                    "type": ty.display(),
                    "expected_type": expected.display() if expected else None,
                    "bindings": {
                        n: {"type": b.ty.display(), "mutable": b.mutable, "constant": b.constant}
    ...
    def call(self, e: Expr, expected: Type | None) -> Type:
        ...
        if "." in n:
            ...
            if name not in self.p.sums or name in self.env:
                fail(
                    "E-CALLEE",
                    "Qualified calls are declared tagged-sum constructors, not methods.",
                    e,
                )
        ...
        if n == "len":
            ...
            e.cpp = (
                "static_cast<std::size_t>(" + t.extent + "ULL)"
                if t.extent.isdigit()
                else "v_" + t.extent
            )
            return Type("usize")
        if n in NUMERIC:
            if len(args) != 1:
                fail("E-ARITY", "Scalar conversion takes one argument.", e)
            ...
            if src.name in INT and n in INT:
                ...
                e.cpp = f"cr::convert<{ty.cpp()}>({args[0].cpp})"
            else:
                e.cpp = f"static_cast<{ty.cpp()}>({args[0].cpp})"
            return ty
        if n in {"add_wrap", "sub_wrap", "mul_wrap", "shl_wrap", "shr"}:
            if len(args) != 2:
                fail("E-ARITY", f"{n} takes two arguments.", e)
            a, b = args
            if a.tag == "int" and b.tag != "int" and n not in {"shl_wrap", "shr"}:
                ...
                self.expr(b, Type("usize") if n in {"shl_wrap", "shr"} else t)
            ...
            if n in {"shl_wrap", "shr"}:
                self.guard("shift")
            e.cpp = f"cr::{n}<{t.cpp()}>({a.cpp}, {b.cpp})"
            return t
        if n in {"min", "max"}:
            if len(args) != 2:
                fail("E-ARITY", f"{n} takes two arguments.", e)
            t = self.expr(args[0], expected)
            self.expr(args[1], t)
            if t.name not in INT or t.mode != "value":
                fail(
                    "E-MINMAX",
                    "Bootstrap min/max are integer-only; floating NaN semantics must be explicit.",
                    e,
                )
            e.cpp = f"std::{n}({args[0].cpp}, {args[1].cpp})"
            return t
        if n in self.p.records:
            fs = self.p.records[n]
            if len(fs) != len(args):
                fail("E-ARITY", f"Record {n} expects {len(fs)} fields in declaration order.", e)
            for a, (_, t) in zip(args, fs):
                self.expr(a, t)
            e.cpp = "ct_" + n + "{" + ", ".join(a.cpp for a in args) + "}"
            return Type(n)
        if n not in self.fs:
            fail("E-CALLEE", f"Unknown callable {n}; arbitrary C++ names are not allowed.", e)
        f = self.fs[n]
        if len(args) != len(f.params):
            fail("E-ARITY", f"{n} expects {len(f.params)} arguments.", e)
            ...
            if t.mode == "value":
                ...
                if not ext.isdigit():
                    ...
                    if (
                        ...
                        and v.val == "len"
                        and len(v.args) == 1
                        and v.args[0].tag == "name"
                    ):
                        view = self.env.get(v.args[0].val)
                        if view is None or view.ty.mode == "value":
                            fail("E-CALL-SHAPE", "len requires a known view.", v)
                        ext = view.ty.extent
                    elif v.tag in {"name", "int"}:
                        ext = v.val
                    else:
                        fail("E-CALL-SHAPE", "View extent must be a name, literal or len(view).", v)
                want = Type(t.name, t.mode, ext)
                if at.mode == "rw" and want.mode == "ro":
                    want = Type(t.name, "rw", ext)
        ...
        assert self.f is not None
        self.call_edges[self.f.name].append(
            (
                n,
                {
                    formal: actual.val
                    for (formal, t), actual in zip(f.params, args)
                    if t.mode != "value"
                },
            )
        )
        self.callset.add(n)
        e.cpp = "cf_" + n + "(" + ", ".join(a.cpp for a in args) + ")"
        return f.ret

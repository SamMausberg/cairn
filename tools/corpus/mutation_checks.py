...
from cairn.agent.hosts.edits import digest
from cairn.compiler.cairnc import compile_source
from cairn.verify.testing import evaluate


def main():
    tasks = json.loads((R / "tools/corpus/lessons/all_tasks_with_oracles.json").read_text())
    muts = {
    ...
    for family, change in muts.items():
        task = next(t for t in tasks if t["id"] == family + "_0")
        old = task["source"]
        new = change(old)
        if old == new:
            raise AssertionError("Mutation failed to change " + family + "\n" + old)
        compile_source(new)

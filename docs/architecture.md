...
The wheel holds the compiler package, the runtime headers, the target support files, the `std` sources, the typed marker and the CLI metadata. Tests, benchmarks, proofs, evidence and historical specifications stay out of it. Ordinary compilation has no third-party Python dependency; Z3, Lean, CUDA and QEMU are optional local tools, and [testing](testing.md) says what their absence does to a gate.

...
def driver(f: Any, sizes: Mapping[str, float], fills: dict[str, str], block_ns: float, blocks: int) -> str:
    ...
    for name, ty in f.params:
        if is_view(ty):
            ...
            element = CPP[ty.name]
            setup.append(f"  Held<{element}> b_{name}({extent});")
            setup.append(f"  for(std::size_t k = 0; k < {extent}; ++k) b_{name}.p[k] = {fill(ty, fills.get(name))};")
            args.append(f"b_{name}.p")
            ...
            raise ValueError(f"{f.name} takes {ty.display()}, which the host timer cannot supply.")
...
def build(source: str, cxx: str, arch: str | None, extra: tuple[str, ...], directory: Path) -> Path:
    ...
    base = [f for f in flags(arch, "exe") if f not in {"-shared", "-fPIC"}]
    obj = directory / "program.o"
    subprocess.run([find(cxx), *base, *extra, "-c", str(directory / "program.cpp"), "-o", str(obj)], check=True,
...
def time(source: str, symbol: str, sizes: Mapping[str, float], *, fills: dict[str, str] | None = None,
    ...
    with tempfile.TemporaryDirectory(prefix="cairn-time-") as scratch:
        ...
        exe = directory / "timed"
        base = [f for f in flags(arch, "exe") if f not in {"-shared", "-fPIC"}]
        subprocess.run([find(cxx), *base, *extra, str(directory / "driver.cpp"), str(obj), "-o", str(exe), "-pthread"],

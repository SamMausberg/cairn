...
def main():
    ...
    try:
        ...
        if a.command=='project':
            print(canonical_source(source),end='');return 0
        if a.command=='check':
            _,r=compile_source(source)
            print(json.dumps({'status':'typed','receipt':r},indent=2));return 0
        if not a.symbol:p.error('--symbol is required for edit commands')
        ...
        if a.command=='packet':result=session.packet(a.site)
        elif a.command=='sites':result=list(session.sites.values())
            ...
            if a.command=='candidates':
                ...
                if not isinstance(request,list):p.error('candidate file must contain an array')
                result=session.candidates(a.site,request)
            else:
                text,result=session.check(request)
                if a.out:
                    if a.out.resolve()==a.source.resolve():p.error('Write a new candidate file; original is read-only.')
                    if a.out.exists():p.error('Output already exists; choose a new candidate path.')
                    a.out.parent.mkdir(parents=True,exist_ok=True)
                    a.out.write_text(text,encoding='utf-8')
                    result['candidate_file']=str(a.out)
        print(json.dumps(result,indent=2,ensure_ascii=False));return 0
    except Diagnostic as e:
        print(json.dumps(explain(e,source),indent=2),file=sys.stderr);return 1
    except (OSError,UnicodeError,RecursionError,ValueError) as e:
        print(json.dumps({'status':'unknown','code':'E-RESOURCE-OR-IO','message':str(e)}),file=sys.stderr);return 2

...
Every pull request fills in every section of `.github/PULL_REQUEST_TEMPLATE.md`: summary, motivation with the roadmap issue it closes or advances, the changes by area, behaviour and compatibility, testing with the commands and what they reported, evidence and claims, and performance. Each section is short and concrete, says `none` where it does not apply, and never keeps the template's own text. Plain sentences, no emojis. Each commit on the branch gets the same care in one line.

Commit messages and pull request titles are one short, plain sentence saying what is now true, with a body only when a reviewer needs it, no emojis and no attribution trailer. The project is licensed MIT or Apache-2.0 at the recipient's option, and every contribution is accepted under the same terms. Publication to a package index or a change of hosting is an explicit owner action, never a side effect of a test or a script.

## Writing documentation

The documentation is [docs/](docs/README.md): fourteen files, one per subject, and the language reference is six of them. Add to the file that owns the subject rather than starting one, keep every file under 800 lines, and update the index when a file is added. README.md says what CAIRN is and how to start it, and nothing that a docs file already says. `docs/std_api.md` and the module pages under `docs/std/` are generated by `make docs` and are never edited by hand. `make site` builds the same files into the website at https://sammausberg.github.io/cairn/, which `.github/workflows/docs.yml` publishes from `main`; a new page also goes into the `nav` of `mkdocs.yml`.

...
from cairn.linear_certificates import Rule, audit_collector, check, collector_rules
...
def render(pairs: tuple[tuple[Rule, object], ...], digest: str) -> str:
    ...
    out("")
    out("Regenerate with:")
    out("    .venv/bin/python tools/export_lean_certificates.py")
    out("Check for drift with:")
    out("    .venv/bin/python tools/export_lean_certificates.py --check")
    out("")
    out("Source of truth: `collector_rules()` in `src/cairn/linear_certificates.py`.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if arguments.check:
        ...
        if current != text:
            ...
            print("regenerate with: python tools/export_lean_certificates.py", file=sys.stderr)
            return 1

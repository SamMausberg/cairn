...
class Type:
    ...
    def cpp(self) -> str:
        ...
        return (
            ("const " if self.mode == "ro" else "") + base + ("*" if self.mode != "value" else "")
        )

    def display(self) -> str:
        return (
            self.name if self.mode == "value" else f"{self.mode}<{self.name}>[{self.extent}]@host"
        )
...
class Parser:
    ...
    def expr(self, prec: int = 0) -> Expr:
        ...
        if self.eat("("):
            ...
            self.i += 1
            e = Expr("unary", t.s, [self.expr(10)], t.line, t.col)
        elif self.eat("true") or self.eat("false"):
            e = Expr("bool", t.s, [], t.line, t.col)
        elif re.fullmatch(r"[0-9]+(?:\.[0-9]+)?(?:[eE][+-]?[0-9]+)?", t.s):
            self.i += 1
            e = Expr("float" if any(c in t.s for c in ".eE") else "int", t.s, [], t.line, t.col)
        else:
            name = self.ident()
            e = Expr("name", name, [], t.line, t.col)
        e.start = self.ts[start].start
        e.end = self.ts[self.i - 1].end
        while True:
            if self.eat("("):
                args = []
                if not self.eat(")"):
                ...
                if e.tag == "field" and e.args[0].tag == "name":
                    ...
                    fail(
                        "E-CALL",
                        "Only direct calls and qualified sum constructors are supported.",
                        e,
                    )
                ...
                idx = self.expr()
                self.need("]")
                e = Expr("index", "", [e, idx], e.line, e.col)
            elif self.eat("."):
    ...
    def stmt(self) -> Stmt:
        ...
        if self.eat("while"):
            e = self.expr()
            b = self.block()
            return Stmt("while", exprs=[e], body=b, line=t.line, col=t.col)
        if self.eat("for"):
            n = self.ident()
            self.need("in")
            lo = self.expr()
        ...
        if self.eat("each"):
            ...
            return Stmt(
                "for",
                n,
                exprs=[Expr("int", "0", line=t.line, col=t.col), hi],
                body=b,
                line=t.line,
                col=t.col,
            )
        ...
        if self.eat("="):
            v = self.expr()
            self.need(";")
            return Stmt("assign", exprs=[e, v], line=t.line, col=t.col)
        self.need(";")
        return Stmt("expr", exprs=[e], line=t.line, col=t.col)
        ...
        while self.t.s != "<eof>":
            ...
            if self.eat("struct"):
                ...
                while not self.eat("}"):
                    ...
                    fs.append((f, typ))
                if n in names:
                    fail("E-DUPLICATE", f"Duplicate declaration {n}.", t)
                names.add(n)
                p.records[n] = fs
            elif self.eat("enum"):
                n = self.ident()
                ...
                if n in names:
                    fail("E-DUPLICATE", f"Duplicate declaration {n}.", t)
                names.add(n)
                if any(ty is not None for _, ty in vs):
                    p.sums[n] = vs
                else:
                    p.enums[n] = [v for v, _ in vs]
            elif self.eat("fn"):
                n = self.ident()
                static = None
                if self.eat("["):
                ...
                fail(
                    "E-DECLARATION",
                    f"Unsupported declaration {self.t.s!r}; expected fn, struct, enum, family, or derive wire.",
                    self.t,
                )
        return p

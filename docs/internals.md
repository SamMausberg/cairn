...
`compiler/cairnc.py` is the facade. `compile_program` runs parse through judge; `generate` calls `audit_collector` (`verify/linear_certificates.py`) before it emits a line, so the collector's one unchecked store never reaches C++ without its seventeen certificates. The `Emitter` asks `audit` (`verify/elision.py`) to accept the proof of every guard the checker discharged before it writes a line, and writes the guard of each one it refuses; `keep_guards` writes every guard.

Checking is one pass per function over one typed tree, and a generic instance is checked as ordinary monomorphic code. One `Checker` class holds the program-wide tables, name and type resolution, generic instances, the whole-program judge and the walk over bodies. Each rule group is a module of functions whose first parameter is the checker, bound as methods by a table in the class so that mypy checks every call, and `s_<tag>` and `e_<tag>` dispatch by the tree node's tag.
...
The device rows are where reuse is still to come, and `cr::gpu::Context` (`cairn_reuse.hpp`, bound to CUDA in `cairn_gpu.hpp`) is what it will come through. A context lends a lane, a stream and its event, until the work queued on it has completed, and gives a returned lane to the next operation instead of making a stream. It hands its one scratch arena to each user ordered after the last on the device, so the host never waits for that. Its budget is declared when it is made: a reserve allocated up front, growth only up to a stated limit, and `Scratch::over_budget`, with nothing queued, beyond it. Stream-ordered allocation is a choice made with the context, never a default. Through a context, a reduction can leave its result on the device for the next kernel (`reduce_to`), and a compaction takes one synchronization where `compact` takes three. It caches no data: every call binds the buffers it is given.

The bookkeeping is tested against a mock device (`tests/runtime/reuse_runtime.cpp`) whose work completes only when the test asks, so an ordering the context forgot shows up as a kernel touching freed memory. The CUDA half compiles for `sm_120` in the everyday suite, and `tests/runtime/gpu_reuse.cu` runs it only under `make gpu`, which has not been run with it. The lowering does not use a context yet: queued device work still takes a stream of its own per ticket, and reduction and compaction still allocate per call, until a device run has checked the context.

`agent/projection.py` prints the canonical read-only projection of the whole language. `agent/agent_tools.py` supplies typed source sites, sealed edit sessions whose effect ceiling can name any effect and whose packet grows by `expand`, and the host that names sessions by handle. `agent/evidence.py` decides what a packet may say is established about each callee, and `agent/diagnostics.py` what a model reads back when a reply is refused, with its fix hint. `agent/state.py` is `cairn state`, `agent/migration.py` the checked interface migration, `agent/plans.py` the plan-only edit and `agent/shot.py` what a program drew. `agent/explain.py` reads the costs `cairn explain` reports from the emitted C++ and clang's optimization record. `agent/sketches.py` binds named choices to ranges and contracts the host owns, and `agent/teaching.py` selects rule cards from lexical tokens. Splicing preserves everything outside the authorized range, every token outside it lexes as it did, and the host rechecks the complete linked module, beyond what the packet displayed.

`perf/` is the performance model, and nothing in it runs a program except `measure.py` on the host and `on_device.py` under the owner's make targets. `work.py` counts what a function does each time it runs, from the typed tree and in its own sizes; `native.py` reads a loop's cost from clang's assembly through llvm-mca, and `device.py` a kernel's registers, spills and instruction mix from ptxas and cuobjdump. `profile.py` is what one machine can do, `calibrate.py` measures it, `model.py` prices the counted work against a profile, `report.py` is `cairn predict`, and `tune.py` is `cairn tune`. The packaged profiles sit in `perf/profiles/`, and a device profile there is a published specification until the owner's calibration replaces it.

`editor/` holds what a person or an editor reads. `lsp.py` is the transport and dispatch of `cairn lsp`, `document.py` one open buffer and `workspace.py` the project it belongs to, analysed whole. Each feature group has a module: `highlighting.py` (semantic tokens), `hints.py` (inlay hints), `navigation.py` (hover and definition), `completion.py`, `edits.py` (formatting, and references and rename within one document), `members.py` (the fields and variants a project's rename reaches), `fixes.py` (quick fixes) and `lenses.py`. `formatting.py` is `cairn fmt`, `docs.py` `cairn doc`, `grammar.py` the generated editor grammars, `terminal.py` what a terminal shows in place of a record, `changes.py` how a `cairn diff` reads, and `shells.py` `cairn completions`. `templates/` holds the projects `cairn new --template` copies.

`verify/linear_certificates.py` checks exact affine implications. `verify/scalar_values.py`, `scalar_symbolic.py`, `scalar_concrete.py` and `scalar_semantics.py` are the value model, the SMT translator, the concrete replay and the query. `verify/verification.py` owns aggregate coverage, and one passing function cannot mark a module checked. `verify/testing.py` runs the JSON task contracts and `verify/runner.py` the test blocks, each in a process of its own. `verify/diff.py` gives each function of two versions its class for `cairn diff`, from `verify/emission.py` (each function's canonical emitted text, and the C++ identities `tools/checks/emission_identity.py` also applies); `projects/revision.py` reads a git revision without a checkout, and `editor/changes.py` renders the record. No agent, test generator or solver may rewrite the authority it is checked against. Native libraries never import the agent tooling or Z3.
...
| `soundness/` | every hole an audit found stays closed, including the two adversarial reviews of 1.4; tasks, leases, atomics, mutexes, host and CUDA lanes, closures; guard elision against its independent audit; plans, fusion, `scan` and rings |
...
`bench/codegen/codegen_only.py` compares code sections by instruction bytes and relocations and implies no timing run. These harnesses write under `results/`, which is ignored and may be replaced on rerun, one subdirectory per kind of output: `native/` (the shared build area), `checks/`, `codegen/`, `lowering/`, `fusion/`, `timing/`, `gpu/`, `host_regions/`, `semantics/`, `systems/`, `density/`, `context/`, `agent/` and `bench_suite/`.

`tools/corpus/` holds inherited teaching fixtures beside the scripts that write and check them: `lessons/` the 0.3 program lessons, `semantic/` the 0.4 same-contract scalar preferences, obligations and protocol repairs. They are inputs kept for audit, regenerated by `curriculum.py`, `semantic_corpus.py` and `protocol_curriculum.py`, and checked by `curriculum_verify.py` and `mutation_checks.py`. No model was trained, and the answers ship beside the tasks. Each generator's `--check` regenerates into a scratch directory and fails when a committed file differs, so a changed rule card, packet or receipt shows up in `tests/tooling/test_tools.py` with the command that regenerates it. The semantic comparison leaves out what records a run rather than a lesson: tool versions, build hashes, the SMT text and the counterexample the solver happened to choose.
...
Outside `unsafe` and `extern`, an accepted program cannot use a moved owner, leak or double-consume a linear value, alias a mutable borrow, keep a borrow past its call, race in a parallel region, touch a place a live task was lent, or index memory of the wrong placement. Arithmetic, bounds, tags, extents and array parts are guarded, and a failed guard aborts the process, on the device as on the host. A guard is left out of the emitted code only where `compiler/facts.py` shows its condition holds, from difference constraints over immutable `usize` values that loop binders, `let` bindings, conditions and early exits supply, and `verify/elision.py` accepts the facts it cites; `tests/soundness/test_established.py` holds both the sites it removes and the counterexamples where it must not, and checks generated programs against an evaluator of its own under AddressSanitizer. `proofs/Cairn/Facts.lean` proves that a guard the rule discharges cannot fail when the facts in scope are true. These rules are implemented in the checker and exercised by rejection tests and by native runs under four sanitizers and device death tests. They are not mechanized: a checker bug is a soundness bug, generic code is checked per instance, and aborts do not run cleanup.
...
A model cannot change a pinned reference, domain, signature, effect ceiling, visible dependency set or source outside its authorized range through an edit response, and the whole linked module is rechecked after every edit. Proof and feedback channels fail closed for missing tools or unsupported fragments. The formatter fails closed: same tokens and comments, or no change. The language server analyses the open buffer, with the other files of its project, in-process and executes nothing; a code lens only names the command the editor runs when a person clicks it.

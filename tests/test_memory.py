...
def test_reject(body,code):
    with pytest.raises(Diagnostic) as e:compile_source('fn f()->u64 {'+body+'}')
...
def test_local_name_collision_does_not_hide_parameter_effect():
    _,r=compile_source('fn g(n:usize,x:rw<u64>[n]){x[0]=1;} fn f(n:usize,out:rw<u64>[n]){if true {buffer x:u64[n]=zeroed;g(n,x);}g(n,out);}')
...
def test_native(cxx,tmp_path):
    ...
    for name in ['sum','initial','early','local_select']:
        ...
        for n in [0,1,2,3,4,7,8,17,64,257]:
            ...
            assert f(n)==expected
    lib.cf_fixed.restype=ctypes.c_uint64;assert lib.cf_fixed()==2
    lib.cf_empty.restype=ctypes.c_size_t;assert lib.cf_empty()==0
    lib.cf_zero_float.restype=ctypes.c_double;assert lib.cf_zero_float()==0.0

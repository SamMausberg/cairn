...
def test_build(tmp_path):
    out = parsed(tool("tools/build.py", "examples/native.cairn", "--out", tmp_path))
...
def test_verify():
    ...
    with exclusive():
        out = parsed(tool("tools/verify.py", "--gcc", "--sanitize", "--tests", "tests/test_compiler.py", timeout=900))
...
def test_codegen_only():
    with exclusive():
        printed = tool("bench/codegen_only.py", timeout=600)
...
def test_density(tmp_path):
    with exclusive():
        printed = tool("tools/density.py", "--output", tmp_path / "density.json")
...
def test_validate_semantics():
    out = parsed(tool("tools/validate_semantics.py", "--gcc", timeout=900))
...
def test_validate_systems():
    out = parsed(tool("tools/validate_systems.py", timeout=900))
...
def test_measure_context(tmp_path):
    printed = parsed(tool("tools/measure_context.py", "--output", tmp_path / "context.json"))
    result = json.loads((tmp_path / "context.json").read_text())
    prior = set(json.loads((ROOT / "bench/fixtures/cards_05.json").read_text())["cards"])
...
def test_mutation_checks():
    printed = tool("tools/mutation_checks.py", timeout=600)
...
def test_sketch_demo(tmp_path):
    out = parsed(tool("tools/sketch_demo.py", "--out", tmp_path, timeout=600))
...
def test_demo(tmp_path):
    out = parsed(tool("tools/demo.py", "--out", tmp_path, timeout=600))
...
def test_agent():
    typed = parsed(tool("tools/agent.py", "check", "examples/agent/selection_before.cairn"))
    assert typed["status"] == "typed" and typed["receipt"]["function_count"] > 0
    packet = parsed(tool("tools/agent.py", "packet", "examples/agent/selection_before.cairn", "--symbol", "select_gt"))
    assert packet["symbol"] == "select_gt" and "base" in packet["rule_cards"]
    sites = json.loads(
        tool("tools/agent.py", "sites", "examples/agent/selection_before.cairn", "--symbol", "select_gt")
    )
    assert sites and all(site["symbol"] == "select_gt" for site in sites)


def test_agent_metrics():
    printed = tool("tools/agent_metrics.py", timeout=600)
...
def test_check_compact_forms():
    result = parsed(tool("tools/check_compact_forms.py"))
...
def test_curriculum(tmp_path):
    result = parsed(tool("tools/curriculum.py", "--out", tmp_path, timeout=600))
...
def test_curriculum_verify():
    printed = tool("tools/curriculum_verify.py", "--gcc", timeout=900)
...
def test_semantic_corpus(tmp_path):
    result = parsed(tool("tools/semantic_corpus.py", "--root", tmp_path, timeout=900))
...
def test_protocol_curriculum(tmp_path):
    ...
    result = parsed(tool("tools/protocol_curriculum.py", "--root", tmp_path, timeout=900))
...
def test_semantic_check():
    ...
    equivalent = parsed(tool("tools/semantic_check.py", base / "reference.cairn", base / "candidate.cairn", "--symbol", "average"))  # fmt: skip
    ...
    unsupported = parsed(tool("tools/semantic_check.py", base / "mixed.cairn", base / "mixed.cairn", "--symbol", "scaled", expect=2))  # fmt: skip
...
def test_task_eval():
    result = parsed(
        tool("tools/task_eval.py", "examples/agent/selection_after.cairn", "--contract", "examples/agent/task.json")
    )
...
def test_publish_private(tmp_path):
    ...
    stopped = parsed(tool("tools/publish_private.py", "not a repository", expect=1))
    assert stopped["status"] == "stopped" and "force push" in stopped["policy"]


def test_export_lean_certificates():
    printed = tool("tools/export_lean_certificates.py", "--check", timeout=600)

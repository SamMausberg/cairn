...
`src/cairn/verify/linear_certificates.py`).  Soundness is universal over integer
...
`tests/verification/test_lean_proofs.py::test_lean_sources_contain_no_escape_hatches`
...
`tools/checks/export_lean_certificates.py` from `collector_rules()`.  Its header records
...
`tools/checks/export_lean_certificates.py --check` — and therefore
`tests/verification/test_lean_proofs.py` — fail.

...
from ..check.traits import vtable
from ..cooperative import cooperative
from ..device import fragments, tensor
...
def check_dyn(c: Checker, e: Expr, args: list[Expr], targs: tuple, expected: Type | None) -> Type:
    ...
    given = c.expr(args[0])
    if is_view(given):  # It holds one value, and a view is not one: `Dyn[T](v[i])` fits.
        c.expect(given, given.value, args[0])
    held = given.value

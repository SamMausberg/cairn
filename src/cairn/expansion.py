        ...
        for field, t in fields:
            ...
            for byte in range(WIDTH[t.name] // 8):
                ...
                enc.append(
                    Stmt(
                        "assign",
                        exprs=[Expr("index", args=[var("out"), lit(offset)]), call("u8", low)],
                    )
                )
            ...
            full = parts[0]
            for part in parts[1:]:
                full = Expr("binary", "|", [full, part])
            decoded.append(full)
        functions = [
            Function(
                "encode_" + record,
                [("out", Type("u8", "rw", str(size))), ("value", Type(record))],
                Type("void"),
                enc,
                source_name="derive wire for " + record,
            ),
            Function(
                "decode_" + record,
                [("input", Type("u8", "ro", str(size)))],
                Type(record),
...
def specialize(p: Program) -> Program:
    ...
    for prefix, name, lo, hi in p.families:
        ...
        if not 0 <= lo < hi <= 2**32 or hi - lo > MAX_FAMILY:
            fail(
                "E-FAMILY-LIMIT",
                "Family must be a nonempty half-open range, at most 1024 variants, below 2^32.",
            )
    ...
    for n in base:
        if not any(name == n for _, name, _, _ in p.families):
            fail(
                "E-UNINSTANTIATED",
                f"Static function {n} has no family; unused templates are not silently ignored.",
            )
    if len(out) > MAX_FUNCTIONS:
        fail("E-EXPANSION-LIMIT", "Expanded program exceeds 2048 functions.")
    p.functions = out
    return p

...
from cairn.verify.scalar.semantics import equivalent, outcome_key, prepared
...
def cases():
        ...
        rows.append(
            {
                ...
                "symbol": "task",
                "assume": "true",
                "allow_reference_traps": False,
                "task": meaning,
                "reference": signature + " {" + reference + "}",
                "chosen": signature + " {" + chosen + "}",
                "rejected": signature + " {" + rejected + "}",
                "split": "evaluation"
                if family in {"floor_average", "saturating_add", "align_eight", "ceiling_div_eight"}
                else "training",
            }
        )
    ...
    for ty in ["u8", "u32", "u64"]:
        ...
        add(
            ...
            un,
            ty,
            f"if x=={m} {{return 0;}} return x+1;",
            "return add_wrap(x,1);",
            "return x+1;",
            f"Return (x+1) modulo 2^{ty[1:]}; do not trap.",
        )
        add(
            "minimum",
            ty,
            ...
            "return min(x,y);",
            "return max(x,y);",
            "Return the smaller input; do not trap.",
        )
        add(
            "maximum",
            ty,
            ...
            "return max(x,y);",
            "return min(x,y);",
            "Return the larger input; do not trap.",
        )
        add(
            "absolute_difference",
            ty,
            ...
            "return x-y;",
            "Return the nonnegative absolute difference; do not trap.",
        )
        add(
            "saturating_add",
            ty,
            p,
            ty,
            f"if y>{m}-x {{return {m};}} return x+y;",
            f"let z=add_wrap(x,y); if z<x {{return {m};}} return z;",
            "return add_wrap(x,y);",
            f"Return min(x+y,{m}) over mathematical integers; do not trap.",
        )
        add(
            "floor_average",
            ty,
            ...
            "return (x&y)+shr(x^y,1);",
            "return add_wrap(x,y)/2;",
            "Return floor((x+y)/2) over mathematical integers for every input, without overflow or traps.",
        )
        add(
            "align_eight",
            ty,
            ...
            f"return x & ~{ty}(7);",
            "return x/8;",
            "Round x down to the greatest multiple of eight not exceeding x; do not trap.",
        )
        add(
            "clear_lowest_bit",
            ty,
            un,
            ty,
            "if x==0 {return 0;} return x&(x-1);",
            "return x&sub_wrap(x,1);",
            "return x^sub_wrap(x,1);",
            "Clear the least significant set bit; zero stays zero; do not trap.",
        )
        add(
            "power_of_two",
            ty,
            un,
            "bool",
            "return x!=0 && (x&sub_wrap(x,1))==0;",
            "if x==0 {return false;} return (x&sub_wrap(x,1))==0;",
            "return (x&sub_wrap(x,1))==0;",
            "Return whether x is a positive power of two. Zero is false; do not trap.",
        )
        add(
            "ceiling_div_eight",
            ty,
            un,
            ty,
            "let base=x/8; if x%8!=0 {return base+1;} return base;",
            "return (x/8)+min(x%8,1);",
            "return add_wrap(x,7)/8;",
            "Return ceil(x/8) over mathematical integers; do not trap.",
        )
        add(
            "low_nibble",
            ty,
            ...
            "return x&15;",
            "return x%15;",
            "Return the low four bits of x; do not trap.",
        )
        add(
            "zero_safe_division",
            ty,
            ...
            "if y!=0 {return x/y;} return 0;",
            "return x/y;",
            "Return zero when y is zero, otherwise floor(x/y); do not trap.",
        )
    for ty in ["i32", "i64"]:
        add(
            "signed_signum",
            ...
            f"let mut z:{ty}=0; if x!=0 {{z=1; if x<0 {{z=-1;}}}} return z;",
            "if x<=0 {return -1;} return 1;",
            "Return -1 for negative, 0 for zero, 1 for positive; do not trap.",
        )
        add(
            "signed_identity",
            ty,
            ...
            "return x+0;",
            "return -(-x);",
            "Return x unchanged for every signed input, including the minimum; do not trap.",
        )
    return rows
...
def write(root: Path) -> dict:
    ...
    for row in rows:
        ...
        for variant in ["chosen", "rejected"]:
            log = []
            r = equivalent(row["reference"], row[variant], "task", query_log=log, timeout_ms=10000)
            expected = "smt-equivalent" if variant == "chosen" else "counterexample"
            if r["status"] != expected:
                raise AssertionError((row["id"], variant, r))
            row[variant + "_receipt"] = r
            for i, q in enumerate(log):
                (queries / f"{row['id']}_{variant}_{i}_{q['stage']}.smt2").write_text(q["smt2"])
    full = "\n\n".join(source)
    (root / "audit_native.cairn").write_text(full)
    native = []
    replay_count = 0
    positive_cases = 0
    rng = random.Random(1709202604)
    all_fns = prepared(full)
    concrete = Concrete(all_fns)
    for compiler in ["clang++", "g++"]:
        with NativeScalar(full, compiler) as lib:
            for row in rows:
                args = row["rejected_receipt"]["counterexample"]
                out = {}
                for variant in ["reference", "chosen", "rejected"]:
                    n = row["id"] + "_" + variant
                    out[variant] = lib.outcome(n, args)
                    assert outcome_key(out[variant]) == outcome_key(concrete.outcome(n, args)), (
                        row["id"],
                        variant,
                        args,
                    )
            ...
            native.append(
                {
                    ...
                    "instrumented_runtime_sha256": lib.runtime_sha256,
                    "generated_sha256": lib.generated_sha256,
                }
            )
    ...
    summary = {
        ...
        "negative_smt_counterexamples": len(rows),
        "native_counterexample_outcomes": replay_count,
        "additional_native_positive_outcomes": positive_cases,
        "native": native,
        "same_signature_same_domain": True,
        "fine_tuning_performed": False,
        "fresh_model_evaluation": False,
...
def main():
    ...
    p.add_argument(
        "--root", type=Path, default=R / "tools/corpus/semantic", help="Where the audited corpus is written."
    )
    p.add_argument(
        "--check", action="store_true", help="Write nothing; exit 1 unless tools/corpus/semantic is current."
    )
    a = p.parse_args()
    if a.check:
        command = "python3 tools/corpus/semantic_corpus.py, then python3 tools/corpus/protocol_curriculum.py"
        return check_generated(write, R / "tools/corpus/semantic", command, erased=RUN, by_name=("obligations",))

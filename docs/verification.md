...
| A declared spread gives every element of its tile exactly one holder, a phase in which every participant writes each of its values writes every element, and with a declared storage layout no two holders write one offset. | Lean, `ok_one_holder`, `ok_covers`, `ok_one_writer`, `storage_distinct`, `ok_no_collision` | `proofs/Cairn/Layout.lean` | That `layouts.py` computes what `Layout.lean` computes: `tools/checks/differential_layouts.py` compares the two on generated layouts. That the lowering computes `D.at(t, v)` as the model's offset, and that a program writes through a spread at all: the rule is about the layout, not the code that uses it. |
| In a phase the cooperative rule accepts, no two threads are ever about to take clashing steps, and every interleaving that runs every thread to its end leaves one memory and one set of registers; a block of accepted phases has one result whatever order its threads take. | Lean, `no_race`, `deterministic`, `block_deterministic` | `proofs/Cairn/Cooperative.lean` | That `phases.py` decides what the model's `program` decides: `tools/checks/differential_cooperative.py` compares the two on generated regions. The global rule of `footprints.py`, warp operations, pipeline stages, and that the host and device lowerings run the modelled steps. |
...
`proofs/` is a dependency-free Lean 4 project, pinned in `proofs/lean-toolchain`, without Mathlib, and `lake build` checks it from scratch in a few seconds. There is no `sorry`, no `native_decide` and no added axiom: every audited declaration depends on `propext` and `Quot.sound` at most.
...
| `Cairn/Layout.lean` | The layout rules of `compiler/layouts.py`: storage layouts and spreads as modes and strides, the element a pair holds, the checks written as their definitions, what a layout that passes promises, and one example of each refusal. |
| `Cairn/Cooperative.lean` | The phase rule of `compiler/phases.py`: steps, phases, interleavings, the executable `accepts`, a thread's run alone, the invariant every interleaving keeps, the three theorems, and the cut-down source (`E`, `C`, `S`) that `program` runs for every thread. |
| `Cairn/Audit.lean` | `#print axioms` for every headline theorem, and the ownership regression line. |

The certificate module is generated from the Python rules, `tools/checks/export_lean_certificates.py --check` fails if the two drift, and the receipt reports `lean_verified: true` only while the live bundle hashes to the one Lean checked. None of this shows that the Python checker implements the Lean `check`, that the emitted C++ is the loop model, or anything about the parser, the type checker, placement, effects, the C++ compiler or the machine.
...
The prefix `out[0..k)` is exactly the selected inputs' projections in order, and the tail is untouched. `store_index_lt_buffer_length` states the bound against the live buffer, so `List.set`, which would silently drop an out-of-range write, never drops one.
...
`Cairn/Places.lean` and `Cairn/Ownership/` are a small ownership-and-lease calculus with an executable checker, an interleaving small-step machine with explicit error states, and safety and progress theorems. It is written by hand beside `src/cairn/compiler/checking.py`, so it shows that the rules are safe as a set. Whether `checking.py` follows them is what the differential harness tests.
...
A single element `a[i]`, a part of a part, and a part with a bound that can change are all modelled conservatively as `elems`: each overlaps every place of its base and adds no `lo <= hi` fact. That these classifications agree with `checking.py` is checked on the named programs of the regression (`tests/soundness/test_soundness.py`), not proved for every program.

### Disjointness, decided twice

The checker decides overlap syntactically, as `checking.py:overlaps` does. Different locals never overlap. The header and the elements do not overlap, which is why `len(d)` stays readable while `d`'s elements are lent. Distinct fields of one record are apart. Two parts of one root are disjoint exactly when one visibly ends at or before the other begins, through a chain of the guarded `lo <= hi` facts of the parts in play (`reaches`). The machine decides overlap under the valuation, and `Ownership.ovl_sound` is the bridge: syntactic disjointness implies real disjointness.
...
A declared layout is held to two rules, and `proofs/Cairn/Layout.lean` states them as their definitions: a spread's every element has exactly one (participant, value) holder, and a storage layout's every element its own offset. `ok_one_holder` and `ok_covers` say a phase in which every participant writes each of its values writes every element once, `ok_one_writer` that no two pairs write one element, and `ok_no_collision` that with a tile that passes its own rule no two pairs write one offset. `gappy_gap`, `doubled_overlap` and `cleared_clash` show each refusal happens.

`compiler/layouts.py` counts holders in one pass and finds a shared offset with a table, where the Lean counts element by element. `tools/checks/differential_layouts.py` asks both about generated layouts, among them what `spread`, `transpose` and `swizzle` make, and requires the same verdict: a coordinate outside the tile, the first element held twice, the first left to nobody, the first two elements that share an offset. Three hundred agree in `make test`, and a planted slip that lets one participant hold an element twice is reported. The rule is about the layout: that a program writes through a spread, and that the lowering's arithmetic is the model's offset, is tested, not proved.

## The phase rule

Between two barriers of a cooperative region, no two threads of a block may touch one element of a shared array where either writes it. `proofs/Cairn/Cooperative.lean` models one block: every thread runs the same phases, and within a phase the threads' steps, loads into registers and stores of values computed from them, interleave in any order. `no_race` says the next steps of two threads of an accepted phase never clash. `deterministic` says any two interleavings that run every thread to its end leave the same memory and the same registers, because every element holds either what its one writer would have put there running alone or what it held before; `block_deterministic` carries that through a block of phases, a barrier between each two.

`compiler/phases.py` decides the rule by running the region's body for every thread of one block, with symbols for what it cannot evaluate. The model's `program` runs a cut-down source for every thread (element reads and writes at indexes built from the thread's number, loop counters and literals, `if` on the thread's number, loops of known count, barriers) and applies `accepts` to each phase. `tools/checks/differential_cooperative.py` generates such regions, renders each as CAIRN and as a Lean term, and requires the checker to accept exactly those `program` accepts; a slip planted in `phases.py` must be reported. That shows agreement on samples, not that the Python is the model. The rule for arrays from outside the region (`footprints.py`), warp operations and pipeline stages are not in the model; the host lowering's thread sanitizer runs and the refusal tests stand for them.
...
Three formal steps remain: proving that the emitted collector loop refines the Lean model; extending the calculus to closures, `lane:f` and placement, and proving of the emitter what the calculus assumes; and relating `checking.py` to the calculus by more than review and a differential run. The receipt's `lean_verified` field covers the certificate bundle alone.

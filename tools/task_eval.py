...
def evaluate(source: str, contract: dict, cxx='clang++') -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix='cairn-task-') as tmp:
        ...
        try:
            ...
            cmd=[sys.executable,str(Path(__file__).resolve()),'--child',str(t/'libtask.so'),str(t/'source.cairn'),str(t/'contract.json')]
            cp=subprocess.run(cmd,text=True,capture_output=True,timeout=8)
            lines=[]

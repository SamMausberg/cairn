...
## Addendum: guards the checker established

This section was committed before any measurement it governs. Everything above stays in force; this adds one build and says how the CAIRN arm is counted.

After 0.8.3 the emitter leaves a guard out where the checker has shown it cannot fail (`src/cairn/compiler/facts.py`), and the receipt lists those sites under `discharged_check_sites`. The CAIRN arm's boundary count is therefore its receipt's `syntactic_check_sites` minus its `discharged_check_sites`, over the same entry point and callees as before. On a kernel whose element accesses are all established, the CAIRN arm no longer carries boundary 2, so it is unequal to the guarded baseline, and the rule above already excludes that pair from every ratio.
...
What this can add to a result is a ratio between CAIRN as it now emits and a C++ baseline that checks exactly what CAIRN still checks. It says nothing new about the guarded column, which keeps the 0.8.3 meaning: the price of every guard the 0.8.3 emitter wrote.

The same work adds a parallel host reduction, `reduce op parallel i in n`, so `sum_u64_wrap` gains a fourth CAIRN arm, `cairn_pool`, whose entry is `sum_u64_pool`. It is counted, checked and timed like the others, and its ratios follow the same rule. The finding the table above predicted for this kernel was that the host `reduce` is sequential; the new arm tests that prediction's successor, which is that a CAIRN reduction on the pool is level with the OpenMP and oneTBB reductions at equal boundaries. `dot_f64` gains no arm, because the pooled form refuses floats.

It also lets a lane own a block of what lanes write, so `histogram_u32` gains a second CAIRN arm, `cairn_blocks`, whose entry is `histogram_u32_blocks`: each lane counts a block of 65536 inputs into its own 256 bins of a scratch buffer the function allocates, and one thread merges them in block order. The block size is fixed here, before any run. The preregistered finding for this kernel was that the parallel shape could not be written; the new arm's ratios test whether the blocked shape is level with the privatized OpenMP and oneTBB histograms, and its scratch allocation is part of what it is timed doing.

An addendum CAIRN arm (`cairn_pool`, `cairn_blocks`) may also be divided into a baseline whose counts it meets or exceeds in every category, because a CAIRN arm that checks more than a baseline and still wins has won with the baseline's checks added. The report marks such a row `cairn checks more`. The primary `cairn` arm keeps the equality rule above. `cairn_blocks` computes its block count and offsets with checked arithmetic outside its inner loop, seven sites, so this is the rule its rows are read under.

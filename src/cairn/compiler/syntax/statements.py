...
from .expressions import ARM_STATEMENTS, REDUCERS, ExpressionParser, copied
...
class StatementParser(ExpressionParser):
    ...
    def stmt(self) -> Stmt:
        ...
        if t.s == "shared" and IDENT.fullmatch(self.ahead(1)) and self.ahead(2) == ":":  # a block's shared array
            ...
            extent = self.expr()
            self.need("]")
            if self.eat(";"):  # no initializer: every element read is written first (compiler/cooperative/phases.py)

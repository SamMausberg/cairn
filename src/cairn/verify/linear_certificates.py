...
def check(rule: Rule, certificate: Certificate) -> bool:
    ...
    if type(rule) is not Rule or type(certificate) is not Certificate:
        return False
    if type(rule.assumptions) is not tuple:
        return False
    ...
    if any(
        ...
        return False
    if type(certificate.weights) is not tuple or len(certificate.weights) != len(rule.assumptions):
        return False
...
def collector_rules() -> tuple[tuple[Rule, Certificate], ...]:
    ...
    for j, goal in enumerate(((1, 1, 0, 0, 0), inv[1], (-1, 0, -1, 1, 0), inv[3])):
        weight = [0] * 5
        weight[0 if j == 0 else 1 if j == 1 else 4 if j == 2 else 3] = 1
...
def audit_collector() -> dict:
    ...
    for rule, certificate in collector_rules():
        if not check(rule, certificate):
            raise ValueError("Invalid collector certificate: " + rule.name)
        rows.append(
            {
                "name": rule.name,
                ...
                "nonnegative_constant": certificate.nonnegative_constant,
            }
        )
    ...
    return {
        ...
        # tools/checks/export_lean_certificates.py regenerates the Lean file and `lake build` passes again.

...
from cairn.agent_tools import EditSession,PROTOCOL
...
def oracle(data):
    if not data:return (3,0,0)
    value=0
    for i,b in enumerate(data):
        if b not in range(48,58):return (1,0,i)
        value=10*value+b-48
        if value >= 2**64:return (2,0,i)
    return (0,value,0)
...
def main():
    ...
    data=[b'',b'0',b'00',b'42',b'18446744073709551615',b'18446744073709551616',
          b'9'*21,b'0'*80,b'1'*80,b'-1',b' 1',bytes([255]),b'12x',b'99\x00']
    ...
    result={'status':'running','seed':18092026,'compilers':{},'finite_tests_only':True,
            'source_sha256':receipt['source_sha256'],'runtime_sha256':receipt['runtime_sha256'],
            'native_timing_performed':False,'lean_verified':False}
    class Observation(C.Structure):_fields_=[('kind',C.c_uint32),('value',C.c_uint64),('offset',C.c_size_t)]
    def execute(args,**kw):
        cp=subprocess.run(args,text=True,capture_output=True,timeout=90,**kw)
        if cp.returncode:raise RuntimeError(json.dumps({'command':args,'returncode':cp.returncode,'stdout':cp.stdout,'stderr':cp.stderr}))
        return cp
    with tempfile.TemporaryDirectory(prefix='cairn-systems-') as directory:
        t=Path(directory);(t/'candidate.cpp').write_text(cpp);(t/'cairn_runtime.hpp').write_text(RUNTIME)
        flags=['-std=c++20','-O2','-ffp-contract=off','-fno-fast-math','-fno-exceptions','-fno-rtti',
               '-Wall','-Wextra','-Werror','-Wno-unused-variable','-Wno-unused-parameter','-Wno-unused-but-set-variable']
        for cxx in ['clang++','g++']:
            so=t/(cxx.replace('+','p')+'.so')
            execute([cxx,*flags,'-shared','-fPIC',str(t/'candidate.cpp'),'-o',str(so)])
            lib=C.CDLL(str(so));lib.cf_observe_decimal.argtypes=[C.c_size_t,C.POINTER(C.c_uint8)];lib.cf_observe_decimal.restype=Observation
            lib.cf_sort_bytes.argtypes=[C.c_size_t,C.POINTER(C.c_uint8),C.POINTER(C.c_uint8)];lib.cf_sort_bytes.restype=None
            lib.cf_sorted_even.argtypes=lib.cf_sort_bytes.argtypes;lib.cf_sorted_even.restype=C.c_size_t
            for raw in data:
            ...
            for raw in arrays:
                n=len(raw);inp=(C.c_uint8*n)(*raw);out=(C.c_uint8*n)(*([173]*n))
                lib.cf_sort_bytes(n,out,inp);assert list(out)==sorted(raw) and bytes(inp)==raw
                out=(C.c_uint8*n)(*([173]*n));count=lib.cf_sorted_even(n,out,inp)
                expected=sorted(x for x in raw if x%2==0)
                assert count==len(expected) and list(out)==expected+[173]*(n-count) and bytes(inp)==raw
            result['compilers'][cxx]={'version':execute([cxx,'--version']).stdout.splitlines()[0],
                'flags':flags,'parser_cases':len(data),'sort_cases':len(arrays),'sorted_filter_cases':len(arrays),
        ...
        owner_cpp,_=compile_source(owners);(t/'owners.cpp').write_text(owner_cpp)
...
'''
        ...
        for cxx in ['clang++','g++']:
            exe=t/'observer';execute([cxx,'-std=c++20','-O0',str(t/'observer.cpp'),'-o',str(exe)])
            execute([str(exe)])
            result['lifetime_observer'][cxx]={'cases':17,'all_allocations_released':True,'instrumented_O0_only':True}
...
'''
        ...
        exe=t/'sanitize';execute(['clang++','-std=c++20','-O1','-g','-fsanitize=address,undefined',
            '-fno-omit-frame-pointer',str(t/'sanitize.cpp'),'-o',str(exe)])
        execute([str(exe)],env={**os.environ,'ASAN_OPTIONS':'detect_leaks=1','UBSAN_OPTIONS':'halt_on_error=1'})
        result['sanitizers']={'compiler':'clang++','array_lengths':1024,'address':True,'undefined':True,'leak':True,
                              'normal_runtime':True,'lifetime_loop_and_match_exits':True}
    ...
    for source in [module('test_memory').SOURCE,module('test_sums').SOURCE,module('test_loop_control').SOURCE]:
        ...
        for name in receipt['functions']:
            ...
            for key,site in session.sites.items():
                changed,_=session.check({'protocol':PROTOCOL,'session':session.session,'kind':'expr','site':key,'replacement':site['source']})

...
from .cairnc import Parser, Diagnostic, fail, IDENT, RESERVED
from .agent_tools import EditSession, PROTOCOL, digest, stable_json, explain, load_json_strict
from .scalar_semantics import equivalent, Concrete, prepared, outcome_key, Unsupported
...
class Sketch:
    ...
    def __init__(self,source: str,symbol: str,*,task: dict|None=None,
                 semantic: ScalarContract|None=None,include: tuple[str,...]=()):
        ...
        self._identity=None
    @property
    def symbol(self): return self._session.symbol
    @property
    def source(self): return self._session.source
    @property
    def semantic(self): return self._semantic
    @property
    def holes(self): return copy.deepcopy(self._holes)

    def hole(self,name: str,original: str,*,occurrence: int|None=None) -> Sketch:
        if self._sealed: fail('E-SKETCH-SEALED','Create a new sketch before changing its slot map.')
        ...
        found=sorted((s for s in self._session.sites.values() if s['source']==original),key=lambda x:(x['start'],x['end']))
        if occurrence is None:
            if len(found)!=1: fail('E-SKETCH-SITE','Slot source must identify exactly one expression; select an occurrence explicitly.',matches=len(found))
            selected=found[0]
        else:
    ...
    def seal(self):
        if not self._holes: fail('E-SKETCH-EMPTY','Declare at least one expression slot.')
        if self._identity is None:
            self._identity=digest(stable_json({'session':self._session.session,'holes':self._holes,
    ...
    def _fresh(self):
        ...
        check=digest(stable_json({'session':self._session.session,'holes':self._holes,
            'semantic':None if self._semantic is None else self._semantic.__dict__}))
        if check!=self._identity:fail('E-SESSION','Sealed sketch content changed.')

    def fill(self,**choices: str) -> Candidate:
        self.seal();self._fresh()
        if set(choices)!=set(self._holes):
            fail('E-SKETCH-CHOICES','Supply each declared slot exactly once, with no extra fields.',
                 missing=sorted(set(self._holes)-set(choices)),extra=sorted(set(choices)-set(self._holes)))
        total=0
        for name,text in choices.items():
            if not isinstance(text,str):fail('E-SKETCH-CHOICES','Every slot value must be an expression string.')
            total+=len(text.encode())
            if total>64000:fail('E-SKETCH-CHOICES','Combined choices exceed 64000 bytes.')
        ...
        text,receipt=self._session.check({'protocol':PROTOCOL,'session':self._session.session,
                                         'kind':'body','replacement':body})
        receipt={**receipt,'sketch_sha256':self._identity,'slots':list(choices),
                 'only_declared_expressions_changed':True,'semantic_status':'not-run'}
    ...
    def packet(self) -> dict:
        ...
        slots={n:{'original':h['source'],'expected_type':h['expected_type'] or h['type'],
                  'bindings':h['bindings']} for n,h in self._holes.items()}
        return {'protocol':'cairn.choices/1','task':old['task'],'source':source,'slots':slots,
                'allowed_effects':old['allowed_effects'],'rule_cards':old['rule_cards'],
                'semantic_contract':None if self._semantic is None else {
                    'reference_source':self._semantic.reference,'symbol':self._semantic.symbol,
                ...
                'acceptance':'Host checks the full module. Types do not establish behavior. '+
                    ('The host also checks a fixed scalar reference.' if self._semantic else 'No semantic reference is installed.'),
    ...
    def check_semantics(self,candidate: Candidate,*,query_log: list|None=None,timeout_ms: int=3000) -> dict:
        self.seal();self._fresh()
        if candidate.receipt.get('sketch_sha256')!=self._identity or digest(candidate.source)!=candidate.receipt.get('candidate_sha256'):
        ...
        c=self._semantic
        return equivalent(c.reference,candidate.source,c.symbol,assume=c.assume,
                          allow_reference_traps=c.allow_reference_traps,timeout_ms=timeout_ms,query_log=query_log)
...
def solve_finite(sketch: Sketch,choices: dict[str,list[str]],*,limit: int=MAX_CHOICES,
                 timeout_ms: int=3000,use_counterexample_cache: bool=True) -> dict:
    ...
    """
    sketch.seal()
    if sketch.semantic is None:fail('E-SKETCH-CONTRACT','Finite semantic search requires a fixed reference.')
    if set(choices)!=set(sketch.holes):fail('E-SKETCH-CHOICES','Search dimensions must equal the declared slots.')
    if type(limit) is not int or not 1<=limit<=MAX_CHOICES:fail('E-SKETCH-BUDGET','Search limit must be 1..4096.')
    total=1
    for xs in choices.values():
        ...
        total*=len(xs)
    if total>limit:fail('E-SKETCH-BUDGET','Candidate product exceeds the explicit search budget.',candidates=total)
    contract=sketch.semantic
    reference=Concrete(prepared(contract.reference))
    witnesses=[]; attempts=[];solver_calls=0;smt_queries=0;cache_rejections=0
    for combination in itertools.product(*choices.values()):
        proposal=dict(zip(choices,combination))
        try: candidate=sketch.fill(**proposal)
        except Diagnostic as e:
            attempts.append({'choices':proposal,'stage':'frontend','result':explain(e)});continue
        cached=None
        if use_counterexample_cache:
            try:
                concrete=Concrete(prepared(candidate.source))
                for inputs in witnesses:
                    expected=reference.outcome(contract.symbol,inputs);actual=concrete.outcome(contract.symbol,inputs)
                    if outcome_key(expected)!=outcome_key(actual):
                        cached={'status':'counterexample','counterexample':inputs,'expected':expected,
                                'actual':actual,'concrete_replay':True};break
            except Unsupported:
        ...
        if result['status']=='smt-equivalent':
            return {'status':'smt-equivalent','mode':'deterministic-finite-search-not-model',
                    'candidate':candidate.source,'choices':proposal,'attempts':attempts,
                    'candidate_product':total,'solver_calls':solver_calls,'smt_queries':smt_queries,'cache_rejections':cache_rejections,
                    'counterexamples':witnesses,'semantic_receipt':result}
    return {'status':'no-certified-candidate','mode':'deterministic-finite-search-not-model',
            'attempts':attempts,'candidate_product':total,'solver_calls':solver_calls,
            'cache_rejections':cache_rejections,'counterexamples':witnesses}

...
The checker answers a type query itself. For each function, `cairn find` writes every call that passes the given values, in any order, with each parameter they leave open as a further value of its declared type, and one check judges them all. A function fits when one of its calls checks, so a hit accepts the given types by the rules a program that calls it meets: the extents a call may leave out stay out, a `Vec` goes where its elements' view is expected, and a template's parameters take the types the values and the wanted result hold. A call that fills every parameter comes before one that leaves some open, and a result of the wanted type comes before an `Option` or a `Result` of it, as `Result[i64, ParseError]` holds `i64`. No other result counts as holding it. The calls are written in the program's root module, so a function private to another module is never a type query's hit, although a word query lists it. A query gives at most four values, since the calls to judge grow as every order of them, and more are refused with `E-REQUEST`.

A word query matches a function's name, its module and the first sentence of its comment, and a few words from other languages count as CAIRN's: `integer` finds `i64`, `string` finds `text` and `thread` finds `spawn`. Two words that spell one of those count as it, so `hash map` finds `std.map`. Only the functions that match the most words are listed, and among those that match as well, one a word names comes first, as `std.sort.sort` does for `sort`. A builtin's rule is code, so each has one line that says what it does, and its effect row belongs to a call: a type query gives it and a word query does not.

...
import sys
import pytest
from cairn.cairnc import compile_source, Diagnostic, Parser
from cairn.agent_tools import EditSession, PROTOCOL, canonical_source
from cairn.sketches import Sketch, ScalarContract
from cairn.scalar_semantics import equivalent
from cairn.project import load_project, ProjectError
from cairn.cli import create_project, main
from cairn.build import build

@pytest.mark.parametrize('body,ty,params', [
    ('add_wrap(x,1)','u64','x:u64'), ('x > 3','bool','x:u64'),
    ('(x & y) + shr(x ^ y,1)','u64','x:u64,y:u64'),
    ('x/2','u32','x:u32'), ('true','bool',''), ('x*2.0','f32','x:f32')])
...
@pytest.mark.parametrize('source', [
    'fn f() = 1;', 'fn f()->void = 1;', 'fn f()->u64 = true;',
    'fn f()->u64 = ;', 'fn f()->u64 = 1', 'fn f()->u64 = 1; return 2;',
    'fn f()->u64 = {return 1;};'])
def test_bad_expression_bodies_rejected(source):
    with pytest.raises(Diagnostic): compile_source(source)
...
def test_host_elision_does_not_accept_other_spaces(space):
    with pytest.raises(Diagnostic): compile_source(f'fn f(x:ro<u64>[1]@{space})->u64=x[0];')
...
def make(tmp_path):
    root=tmp_path/'demo';create_project(root);return root

def test_project_is_ordered_and_pinned(tmp_path):
    root=make(tmp_path);p=load_project(root)
...
def test_project_error_maps_to_original_file(tmp_path):
    root=make(tmp_path);(root/'src/main.cairn').write_text('fn main()->i32 {\nreturn true;\n}')
    p=load_project(root)
    with pytest.raises(Diagnostic) as exc: compile_source(p.source)
...
def test_bad_manifests_fail_closed(tmp_path,manifest):
    root=make(tmp_path);(root/'cairn.toml').write_text(manifest)
    with pytest.raises(ProjectError):load_project(root)

def test_symlink_source_rejected(tmp_path):
    root=make(tmp_path);(root/'src/math.cairn').unlink()
    external=tmp_path/'external.cairn';external.write_text('fn x()->u64=0;')
...
def test_new_never_overwrites(tmp_path):
    ...
    with pytest.raises(FileExistsError):create_project(root)
    assert before==(root/'cairn.toml').read_bytes()

@pytest.mark.parametrize('cxx',['clang++','g++'])
def test_native_project_and_tests(tmp_path,cxx):
    import shutil
    if not shutil.which(cxx):pytest.skip('Native compiler unavailable')
    root=make(tmp_path);record=build(load_project(root),cxx=cxx)
...
def test_unknown_tool_does_not_produce_output(tmp_path):
    root=make(tmp_path)
    with pytest.raises(ProjectError):build(load_project(root),cxx='nonexistent-cairn-compiler')
...
def test_bad_executable_entry(tmp_path,src):
    p=tmp_path/'bad.cairn';p.write_text(src)
    with pytest.raises(ProjectError):build(load_project(p),kind='exe')

def test_output_symlink_rejected(tmp_path):
    root=make(tmp_path); elsewhere=tmp_path/'elsewhere';elsewhere.mkdir()
    (root/'build').symlink_to(elsewhere,target_is_directory=True)
    with pytest.raises(ProjectError):build(load_project(root))

...
from cairn.formatting import comments, format_report, format_source
from cairn.syntax import TOKEN, Diagnostic, lex
...
def repository_sources():
    return sorted(p for p in ROOT.rglob("*.cairn") if ".venv" not in p.parts)


def embedded_programs():
    """CAIRN programs written inline in the test suite, recognised by shape and lexed."""
    out = []
    for path in sorted(ROOT.joinpath("tests").glob("*.py")):

...
The adapter reads `harness.toml` (projects/harness/mapping.py), writes the export (projects/export.py) and the
submission around it (projects/harness/sources.py), and records the format and the upstream revision it follows,
...
from .mapping import Mapping, load
from .sources import Library, binding, gpumode, kernelbench, python, signature, solution

...
- A recipe may declare a field extent in a record it generates (`$f:Buf[$t][rows]`), held by the same `E-EXTENT` and `E-EXTENT-FIELD` rules as a written record, so a generated column goes to a call whole.

...
The bookkeeping is tested against a mock device (`tests/native/reuse_runtime.cpp`) whose work completes only when the test asks, so an ordering the context forgot shows up as a kernel touching freed memory. The CUDA half compiles for `sm_120` in the everyday suite, and `tests/native/gpu_reuse.cu` runs it only under `make gpu`, which has not been run with it. The lowering does not use a context yet: queued device work still takes a stream of its own per ticket, and reduction and compaction still allocate per call, until a device run has checked the context.

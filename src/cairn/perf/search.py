...
2. Each legal candidate is counted from its own checked program, priced by the model at every size and ranked by
   the objective over them (`perf/objective.py`). It is lowered as `cairn build` lowers it, and one whose function
...
(`perf/tune.py` times; device plans only under the owner's make target). The clock is read before every step. A

...
INTRINSIC_TYPES = {"Buf": 1, "Array": 2, "fn": None, "dyn": 1, "Dyn": 1, "Ticket": 1, "Atomic": 1, "Mutex": 1, "Group": 1,
                   "IoRing": 0}  # fmt: skip
# Tensor-core fragments (compiler/device/fragments.py): an element type and M, N, K.
...
class Stmt:
    ...
    fuse: int = 0  # How many adjacent regions, this one first, a plan lets run as one traversal (plans/fusion.py).
    vector: int = 0  # Adjacent indices each lane of a device region runs over W-element chunks (plans/chunks.py).
    chunked: tuple = ()  # The arrays a vectored region moves W at a time: (name, element, loaded, stored, view).
    stage: int = 0  # How far either side of a device block's indices its tiles reach (compiler/plans/staging.py).
...
class Implements:
    ...
    value `tune` lists for each of its natural parameters (compiler/plans/implementations.py)."""
...
class Function:
    ...
    launch: tuple[str, int] | None = None  # (threads, block) of an extern that is a CUDA kernel (device/launches.py)
...
class Program:
    ...
    layouts: dict[str, Expr] = field(default_factory=dict)  # `layout NAME = ...;`, which device/layouts.py folds

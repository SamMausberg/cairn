#!/usr/bin/env python3
"""Compile and run bench/host_tasks/host_tasks.cpp against one or two sets of runtime headers, interleaved.
...
def compile_against(cxx: str, runtime: Path, arch: str, exe: Path) -> list[str]:
    line = [cxx, *profile_flags("exe", arch), "-pthread", f"-I{runtime}", str(ROOT / "bench/host_tasks/host_tasks.cpp")]

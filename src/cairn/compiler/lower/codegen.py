...
RUNTIME_FILES = {
    p.name: p.read_text(encoding="utf-8") for p in sorted((Path(__file__).parents[2] / "runtime").glob("*.hpp"))
}
...
class Emitter:
    ...
    def type(self, t: Type) -> str:
        ...
        elif t.name in fragments.TYPES:  # a warp's tensor-core fragment (compiler/device/fragments.py)
    ...
    def exported(self, f: Function) -> bool:
        ...
        function value. compiler/lower/header.py declares exactly these."""

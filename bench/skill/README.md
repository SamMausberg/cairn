...
A `claude plugin eval` suite, which `.claude-plugin/plugin.json` names under `experimental.evals`. Each directory is one case: `prompt.md` with its allowed tools in the frontmatter, and `graders/`. Five cases give a refused program and grade the diagnostic code by a regular expression, the fix by a rubric, and whether the skill fired; `write-checksum` asks for a program. The cases use only the Read, Glob, Grep and Skill tools.

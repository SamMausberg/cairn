...
| `guard_counts.py` | the guards the emitter writes for every example, `std` and the bench kernels, as emitted and with every guard kept, and the entry checks calls no longer run | nothing beyond the package | `results/lowering/guard_counts.json` |

...
The host opens a session on `bar_x` with the task "the colour bar hides the right edge of the plate". The packet holds the source of `bar_x`, the constant it uses, and the signature and effect row of its caller `frame`. The agent asks for a shot, and the host builds the program, runs it headless and returns each frame with its layout. The agent reads the overlap from numbers, never from pixels:
...
The second shot shows the bar clear of the map. `changed rows: {}` says the edit left the effect row of `frame` as it was: drawing a frame allocates, frees and runs a host region as before, and gains nothing such as `io`. The fixed program passes its test. These are its frames after 40, 1000 and 5000 sweeps:

...
def main():
    ...
    ap.add_argument("--obligations", type=Path)
    a = ap.parse_args()
    queries = []
    try:
        r = equivalent(
            a.reference.read_text(),
            a.candidate.read_text(),
            a.symbol,
            assume=a.assume,
            allow_reference_traps=a.allow_reference_traps,
        ...
        if a.obligations:
            ...
            for i, q in enumerate(queries):
                (a.obligations / f"{i}_{q['stage']}.smt2").write_text(q["smt2"])
        print(json.dumps(r, indent=2))
        return 0 if r["status"] == "smt-equivalent" else 1 if r["status"] == "counterexample" else 2
    except (OSError, ValueError) as e:
        print(json.dumps({"status": "unknown", "reason": str(e)}))
        return 2

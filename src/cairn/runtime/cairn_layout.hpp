// Layouts in code (compiler/device/layouts.py). `L.at(r, c)`, `D.row(t, v)` and their siblings lower to the arithmetic a
...
// A fragment's tile (compiler/device/fragments.py): its array holds every offset the layout places, or the operation
...
// compiler/device/layouts.py refuses a layout under which two elements share an offset before any code is written.

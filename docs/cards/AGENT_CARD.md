# CAIRN Native 0.5 bootstrap card
...
Expression body: fn inc(x:u64)->u64 = add_wrap(x,1); is exactly one return.
View @host may be omitted in this CPU profile; no transfer is inferred.
else if is nested branching. Parameters are immutable. for i in 0..n visits the half-open range sequentially;

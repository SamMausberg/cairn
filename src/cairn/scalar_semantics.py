...
from .cairnc import Parser, Checker, Function, Expr, Stmt, Type, INT, SIGNED, WIDTH, Diagnostic
...
class ConcreteTrap(Exception): pass
...
def conj(*parts: str) -> str:
    if 'false' in parts: return 'false'
...
def same(a: str,b: str) -> str: return 'true' if a == b else f'(= {a} {b})'
...
def extend(value: str, source_width: int, dest_width: int, signed: bool) -> str:
    if source_width==dest_width: return value
    if source_width>dest_width: return f'((_ extract {dest_width-1} 0) {value})'
...
class Formula:
    ...
    def bind(self,expression: str,ty: str) -> str:
        ...
        if len(self.definitions)>=MAX_TERMS: raise Unsupported('Symbolic term budget exceeded.')
...
class Symbolic:
    ...
    def tick(self):
        self.visits+=1
        if self.visits>MAX_TERMS: raise Unsupported('Symbolic evaluation budget exceeded.')
    def expr(self,e: Expr,env: dict[str,Term],stack: tuple[str,...]) -> Term:
        ...
        ty=e.ty.name
        if e.tag=='name': return env[e.val]
        if e.tag=='int': return Term(ty,constant(int(e.val),ty))
        if e.tag=='bool': return Term('bool',e.val)
        if e.tag=='unary':
            a=self.expr(e.args[0],env,stack)
            if e.val=='!': return self.q.term('bool',neg(a.value),a.defined)
            if e.val=='~': return self.q.term(ty,f'(bvnot {a.value})',a.defined)
        ...
        if e.tag=='binary':
            a=self.expr(e.args[0],env,stack); b=self.expr(e.args[1],env,stack)
            op=e.val; both=conj(a.defined,b.defined)
            if op=='&&':
            ...
            if op in {'<','<=','>','>='}:
                if a.ty=='bool':
                    values={'<':conj(neg(a.value),b.value),'<=':disj(neg(a.value),b.value),
                            '>':conj(a.value,neg(b.value)),'>=':disj(a.value,neg(b.value))}
                    v=values[op]
                else:
            ...
            if op in {'+','-','*'}: return self.arithmetic(ty,op,a,b,checked=True)
            if op in {'/','%'}:
                ok=neg(same(b.value,constant(0,ty)))
                if ty in SIGNED:
                    ok=conj(ok,neg(conj(same(a.value,constant(bounds(ty)[0],ty)),same(b.value,constant(-1,ty)))))
        ...
        if e.tag=='call':
            ...
            ok=conj(*(a.defined for a in args)); n=e.val
            if n in INT:
                a=args[0]; ws=WIDTH[a.ty]; wt=WIDTH[n]; common=max(ws,wt)+1
            ...
            if n in {'shl_wrap','shr'}:
                a,b=args; width=WIDTH[ty]
                limit=f'(bvult {b.value} {constant(width,b.ty)})'
                shift=extend(b.value,WIDTH[b.ty],width,False)
                return self.q.term(ty,f'({"bvshl" if n=="shl_wrap" else "bvlshr"} {a.value} {shift})',conj(ok,limit))
            if n in {'min','max'}:
                a,b=args; cmp=f'(bv{"s" if ty in SIGNED else "u"}le {a.value} {b.value})'
    ...
    def arithmetic(self,ty: str,op: str,a: Term,b: Term,checked: bool) -> Term:
        ...
        value=f'({fn} {a.value} {b.value})'; ok=conj(a.defined,b.defined)
        if checked:
            w=WIDTH[ty]; signed=ty in SIGNED
    ...
    def invoke(self,name: str,args: list[Term],stack: tuple[str,...]=()) -> Term:
        if name in stack: raise Unsupported('Recursive functions are not modeled.')
        if len(stack)>24: raise Unsupported('Call-depth limit exceeded.')
        ...
        if remaining: raise Unsupported('A scalar function fell through without returning.')
        default='false' if f.ret.name=='bool' else constant(0,f.ret.name)
        value=default
        for path,v in reversed(returns): value=ite(path,v.value,value)
    ...
    def block(self,body: list[Stmt],states: list,stack: tuple[str,...],returns: list) -> list:
        ...
        for s in body:
            self.tick()
            if len(states)+len(returns)>MAX_PATHS: raise Unsupported('Path budget exceeded.')
            next_states=[]
            for path,env in states:
                ...
                else: raise Unsupported('Only scalar locals, assignment, if/else and value returns are modeled; no loops.')
            states=next_states
...
class Concrete:
    ...
    """
    def __init__(self,functions: dict[str,Function]): self.functions=functions
    def checked(self,value,ty):
        lo,hi=bounds(ty)
        if not lo<=value<=hi: raise ConcreteTrap('integer-overflow-or-conversion')
        return value
    def expr(self,e,env,stack):
        if e.tag=='name': return env[e.val]
        if e.tag=='int': return int(e.val)
        if e.tag=='bool': return e.val=='true'
        ty=e.ty.name
        if e.tag=='unary':
            a=self.expr(e.args[0],env,stack)
            if e.val=='!': return not a
            if e.val=='~': return (~a)&((1<<WIDTH[ty])-1)
            if e.val=='-': return self.checked(-a,ty)
        if e.tag=='binary':
            a=self.expr(e.args[0],env,stack); op=e.val
            if op=='&&': return a and self.expr(e.args[1],env,stack)
            if op=='||': return a or self.expr(e.args[1],env,stack)
            ...
            if op=='|': return a|b
            if op=='^': return a^b
            if op=='+': return self.checked(a+b,ty)
            if op=='-': return self.checked(a-b,ty)
            if op=='*': return self.checked(a*b,ty)
            if op in {'/','%'}:
                if b==0 or (ty in SIGNED and a==bounds(ty)[0] and b==-1): raise ConcreteTrap('invalid-division')
                q=abs(a)//abs(b)
                if (a<0)!=(b<0): q=-q
                return q if op=='/' else a-q*b
        if e.tag=='call':
            xs=[self.expr(a,env,stack) for a in e.args];n=e.val
            if n in INT: return self.checked(xs[0],n)
            if n in {'min','max'}: return (min if n=='min' else max)(*xs)
            if n in {'add_wrap','sub_wrap','mul_wrap'}:
                a,b=xs;v=a+b if n=='add_wrap' else a-b if n=='sub_wrap' else a*b
                return v% (1<<WIDTH[ty])
            if n in {'shl_wrap','shr'}:
                a,b=xs
                if not 0<=b<WIDTH[ty]: raise ConcreteTrap('invalid-shift')
                return ((a<<b)%(1<<WIDTH[ty])) if n=='shl_wrap' else (a>>b)
            if n in self.functions: return self.invoke(n,xs,stack)
        raise Unsupported('Concrete replay encountered unsupported expression.')
    def invoke(self,name,args,stack=()):
        if name in stack: raise Unsupported('Concrete replay does not recurse.')
        f=self.functions[name];env={n:a for (n,_),a in zip(f.params,args)}
        ret,value=self.block(f.body,env,stack+(name,))
        if not ret: raise Unsupported('Concrete scalar function did not return.')
        return value
    def block(self,body,env,stack):
        old=set(env)
        for s in body:
            if s.tag in {'let','reg'}: env[s.name]=self.expr(s.exprs[0],env,stack)
            elif s.tag=='assign' and s.exprs[0].tag=='name': env[s.exprs[0].val]=self.expr(s.exprs[1],env,stack)
            elif s.tag=='return': return True,self.expr(s.exprs[0],env,stack)
            elif s.tag=='if':
            ...
            else: raise Unsupported('Concrete replay does not execute this statement.')
        for k in set(env)-old: del env[k]
        return False,None
    def outcome(self,name,args: dict[str,Any]):
        f=self.functions[name]
        if set(args)!={n for n,_ in f.params}: raise ValueError('Wrong scalar argument names.')
        for n,t in f.params:
            if t.name=='bool':
                if type(args[n]) is not bool: raise ValueError('Boolean input required.')
        ...
        try: return {'defined':True,'return':self.invoke(name,[args[n] for n,_ in f.params])}
        except ConcreteTrap as e: return {'defined':False,'trap':str(e)}

def prepared(source: str):
    if len(source.encode())>MAX_SOURCE_BYTES: raise Unsupported('Scalar source limit exceeded.')
...
def implementation_hash():
    ...
    return hashlib.sha256(b''.join((parent/n).read_bytes() for n in ['scalar_semantics.py','smt_bridge.py','cairnc.py','syntax.py','checking.py','expansion.py','codegen.py','version.py'])).hexdigest()
...
def equivalent(reference: str,candidate: str,symbol: str,*,assume: str='true',
               ...
               query_log: list|None=None) -> dict[str,Any]:
    ...
    if not all(isinstance(x,str) for x in (reference,candidate,symbol,assume)):
        return {'protocol':'cairn.semantic/1','status':'invalid-contract',
                'reason':'Sources, symbol and precondition must be strings.',
                'lean_verified':False,'native_verified':False}
    common={'protocol':'cairn.semantic/1','profile':PROFILE,
            'reference_sha256':sha(reference),'candidate_sha256':sha(candidate),
            'symbol':symbol,'assume':assume,'allow_reference_traps':allow_reference_traps,
            'implementation_sha256':implementation_hash(),
            'trust':['CAIRN parser/typechecker','scalar SMT translation','Z3 solver'],
            'lean_verified':False,'native_verified':False}
    if type(allow_reference_traps) is not bool: return {**common,'status':'invalid-contract','reason':'Trap policy must be Boolean.'}
    try:
        precondition_parser=Parser(assume)
        precondition_parser.expr();precondition_parser.need('<eof>')
        refs=prepared(reference);cands=prepared(candidate)
        if symbol not in refs or symbol not in cands: raise Unsupported('Selected scalar symbol not found.')
        rf=refs[symbol];cf=cands[symbol]
        if rf.params!=cf.params or rf.ret!=cf.ret:
            return {**common,'status':'invalid-contract','reason':'Candidate signature differs from fixed reference.'}
        q=Formula(rf.params)
        ...
        with Solver(timeout_ms) as solver:
            def run(stage,assertion):
                ...
                if query_log is not None: query_log.append({'stage':stage,'smt2':text+'(check-sat)\n','result':result})
                return result
            def finish(status,**fields):
                return {**common,'status':status,'solver_version':solver.version,
                        'queries':query_summaries,**fields}
            def inputs(result): return {n:result['values'][f'arg_{i}'] for i,n in enumerate(input_names)}
            if domain.defined!='true':
                r=run('domain-totality',neg(domain.defined))
                if r['status']=='sat':return finish('invalid-domain',reason='Precondition may trap.',counterexample=inputs(r))
                if r['status']!='unsat':return finish('unknown',reason='Domain totality was not established.')
            if domain.value!='true':
                r=run('domain-nonempty',domain.value)
                if r['status']=='unsat':return finish('invalid-domain',reason='Precondition admits no inputs; vacuous acceptance rejected.')
                if r['status']!='sat':return finish('unknown',reason='Nonempty domain was not established.')
            if not allow_reference_traps:
                r=run('reference-totality',conj(domain.value,neg(left.defined)))
                if r['status']=='sat':return finish('invalid-reference',reason='Reference traps on an admitted input.',counterexample=inputs(r))
                if r['status']!='unsat':return finish('unknown',reason='Reference totality was not established.')
            mismatch=disj(neg(same(left.defined,right.defined)),conj(left.defined,right.defined,neg(same(left.value,right.value))))
            r=run('equivalence',conj(domain.value,mismatch))
            if r['status']=='unsat':
                return finish('smt-equivalent',quantification='All declared-width scalar inputs satisfying the host precondition.',
                              observation='Return value, or one undifferentiated abort outcome; no memory/timing observation.')
            if r['status']!='sat': return finish('unknown',reason='Equivalence solver did not decide the obligation.')
            args=inputs(r)
            ...
            if outcome_key(expected)==outcome_key(actual):
                return finish('unknown',reason='Solver/concrete replay disagreed; no semantic rejection is certified.',counterexample=args)
            return finish('counterexample',counterexample=args,expected=expected,actual=actual,
                          concrete_replay=True,native_replay='not-run')
    except Diagnostic as e: return {**common,'status':'rejected','diagnostic':e.data}
    except Unsupported as e: return {**common,'status':'unknown','reason':str(e),'unsupported_profile':True}

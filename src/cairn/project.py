...
class ProjectError(ValueError):
    """A malformed or unsafe project description, not a compiler proof failure."""
...
def load_project(path: str | Path = '.') -> Project:
    ...
    if target.suffix == '.cairn':
        ...
        return Project(root, target.stem, body,
    ...
    kind, arch = build.get('kind', 'library'), build.get('arch', 'x86-64')
    if not isinstance(kind,str) or not isinstance(arch,str) or kind not in {'library', 'exe'} or arch not in {'x86-64', 'x86-64-v3'}:
    ...
    return Project(root, name, combined, tuple(units), tuple(contracts), kind, arch,

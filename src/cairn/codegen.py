...
from .checking import COMPARISONS, WRAPPING, Checker, is_view, root
from .syntax import CPP, FLOAT, INT, NUMERIC, SIGNED, VERSION, VOID, Expr, Function, Program, Stmt, Type

RUNTIME_FILES = {p.name: p.read_text(encoding="utf-8") for p in sorted((Path(__file__).parent / "runtime").glob("*.hpp"))}
...
class Emitter:
    ...
    def s_let(self, s: Stmt, es: list[str]):
        const = "const " if s.tag == "let" and (self.trivial(s.ty) or s.ty.mode != "value") else ""
    ...
    def s_return(self, s: Stmt, es: list[str]):
        self.put("return" + (" " + es[0] if es else "") + ";")
...
__all__ = ["RUNTIME", "RUNTIME_FILES", "Emitter", "mangle"]
_ = (Any, SIGNED, VOID, root)

...
$ cairn tune results/demos/implement/sumsq --symbol sumsq --at n=65536 --measure 6 --budget-seconds 120 --budget-runs 16 --write
  ...
  chosen: plan sumsq use sumsq_blocks[8];
  measured, median of 3 blocks: (no plan for sumsq) 46.2 us, plan sumsq use sumsq_blocks[32]; 20.6 us, plan sumsq use sumsq_blocks[16]; 18.9 us, plan sumsq use sumsq_blocks[8]; 12.2 us, plan sumsq use sumsq_blocks[4]; 12.3 us, plan sumsq use sumsq_by4; 11.8 us
  measured, median of 6 blocks: plan sumsq use sumsq_by4; 13.1 us, plan sumsq use sumsq_blocks[8]; 11 us, plan sumsq use sumsq_blocks[4]; 12.6 us
  measured, median of 12 blocks: plan sumsq use sumsq_blocks[8]; 12.5 us, plan sumsq use sumsq_blocks[4]; 12.9 us
  budget: 0 of 4 compiles, 0 kept; 11.74 s of 120.0; 11 runs, 0 kept
  11 runs started of the 16 allowed; timed on this host (AMD Ryzen 7 7800X3D 8-Core Processor, 16 threads),
  which other work shared: load average 9.1 before, 7.4 after
  --write put plan sumsq use sumsq_blocks[8]; into src/sumsq.cairn
```

The model ranks the reference first and `sumsq_by4` last. Measured, the reference is the slowest by about four times, and the search's answer counts 3 of 15 measured pairs in the predicted order. `cairn tune --compare` reports the difference between the reference and the chosen instance, each line labelled by the kind of evidence it is:

```text
$ cairn tune results/demos/implement/sumsq --symbol sumsq --at n=65536 --compare none --compare "use sumsq_blocks[8]"
  sumsq: (no plan for sumsq)  ->  plan sumsq use sumsq_blocks[8];
    [compiler observation] cairn predict (the model, not a run): at n=65536: predicted 3.705e+04 ns -> 4.168e+04 ns; the model's bound compute -> compute; confidence high
    [runtime measurement] cairn.perf.measure: the candidate built with the project's flags beside a driver that fills each view, timed in blocks of at least 2 ms, the median of 3 blocks, on this host: a at n=65536: median 4.62e+04 ns (min 4.54e+04, max 7.418e+04)
    [runtime measurement] cairn.perf.measure: the candidate built with the project's flags beside a driver that fills each view, timed in blocks of at least 2 ms, the median of 3 blocks, on this host: b at n=65536: median 1.217e+04 ns (min 1.194e+04, max 1.225e+04)
    [runtime measurement] cairn.perf.measure: the candidate built with the project's flags beside a driver that fills each view, timed in blocks of at least 2 ms, the median of 6 blocks, on this host: b at n=65536: median 1.104e+04 ns (min 1.078e+04, max 1.151e+04)
    [runtime measurement] cairn.perf.measure: the candidate built with the project's flags beside a driver that fills each view, timed in blocks of at least 2 ms, the median of 12 blocks, on this host: b at n=65536: median 1.252e+04 ns (min 1.102e+04, max 1.328e+04)
    [hypothesis] the model against the measurement: the model predicted x1.12 and the runs measured x0.271: the model leaves out what separates them
...
A validation is finite testing on the cases that ran, never proof: the reference and each implementation are compiled by the same compiler, so a fault they share would agree with itself. Z3 is asked apart from the tests, and its query covers only n <= 16, its unrolling bound; it answers `unknown` for `sumsq_by4` and three of the instances, and `smt-equivalent` for `sumsq_blocks[32]`, where that bound leaves only n = 0.

...
for k in range(1,257):
    for n in [0,1,7,31,257]:
        ...
        a=(C.c_float*n)(*vals); b=(C.c_float*n)()
        f(k,n,b,a)
        assert list(b)==[C.c_float(v*k).value for v in vals]
        ncases+=1
print(json.dumps({'cpp_template_family_cases':ncases,'all_passed':True,
 'scope':'Same family values; indexed C++ entry versus named CAIRN entries. Not an ABI or performance equivalence test.'},indent=2))

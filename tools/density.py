...
def main():
    ...
    if args.tiktoken:
        ...
        encode=enc.encode; label='tiktoken/'+args.tiktoken
    else:
        def encode(text): return [b+3 for b in text.encode('utf-8')]
    ...
    def count(t): return len(encode(t))
    names=['saxpy','dot','sum_wrap','prefix','count_gt','histogram','compact_even','lower_bound','gcd']
    def extract(src,pattern):
        m=re.search(pattern,src)
        if not m: raise RuntimeError('Function not found: '+pattern)
        start=m.start(); brace=src.index('{',m.end()-1); depth=1; end=brace+1
        while depth:
            depth += (src[end]=='{')-(src[end]=='}'); end+=1
        return src[start:end]+'\n'
    native=(R/'examples/native.cairn').read_text(); ref=(R/'bench/reference.cpp').read_text()
    pairs=[]
    for n in names:
        a=extract(native,r'fn '+n+r'\(')
        b=extract(ref,r'extern "C" [^\n]*?cc_'+n+r'\(')
        pairs.append({'function':n,'cairn_tokens':count(a),'cpp_tokens':count(b),
          'cpp_over_cairn':count(b)/count(a),'cairn_sha256':hashlib.sha256(a.encode()).hexdigest(),

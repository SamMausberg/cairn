# CAIRN 0.6 complete feature card

Generated from the compiler's actual feature cards. Packets select only relevant cards. This full card deliberately includes more features than 0.5. No model training or proficiency result is implied.

## base

CAIRN 0.6 is a checked CPU prototype, not Rust/Python. Use fn, typed interfaces,
...
Types: bool, u8/u16/u32/u64, usize(64-bit), i32/i64. +,-,* trap on overflow in
...
ro<T>[n] and rw<T>[n] borrow host storage; @host is optional, not a transfer.
...
Call declared functions, not invented libraries. Calls with external writes or
...
f32/f64 use -ffp-contract=off -fno-fast-math. No implicit reassociation or FMA;
...
struct Pair {x:u64; y:u64;} is a value record. Construct Pair(a,b), access p.x.
...
A function may declare one [K:nat] parameter. family gain=scale[1..257];
...
buffer scratch:u64[n] = zeroed; explicitly allocates and zero-initializes heap
...
enum Result {Value(u64); Error(u32); Empty;} defines a monomorphic tagged sum.

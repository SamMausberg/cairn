...
def contract(tmp_path: Path, cpp: str, cxx: str, *extra: str, cuda=False, timeout=240, env=None, under=(),
             emulate=False, entry: str | None = "main", beside=None, stand_in=None, input=None):  # fmt: skip
    """`cpp`, with the C++ files `beside` it, built by the project's own command line for `cxx` plus `extra`, then run
    once `under` a wrapper, with `input` on its standard input. With `emulate`, a device program is built for the host,
    its device work on host threads (projects/emulation.py)."""
    if not shutil.which(cxx):
        pytest.skip(f"{cxx} unavailable")
    ...
    return subprocess.run([*under, executable], capture_output=True, text=True, timeout=timeout, env=env, input=input)

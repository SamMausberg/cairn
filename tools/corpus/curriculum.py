...
from cairn.agent.hosts.edits import explain, stable_json
...
def write(root: Path) -> dict:
    ...
    summary = {
        ...
        "native_validation": "run tools/corpus/curriculum_verify.py; generation checks frontend only",
...
def main():
    ...
    p.add_argument("--out", type=Path, default=R / "tools/corpus/lessons", help="Where the teaching data is written.")
    p.add_argument("--check", action="store_true", help="Write nothing; exit 1 unless tools/corpus/lessons is current.")
    a = p.parse_args()
    if a.check:
        return check_generated(write, R / "tools/corpus/lessons", "python3 tools/corpus/curriculum.py")

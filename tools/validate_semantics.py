...
def fixtures():
    ...
    def add(name,ty,expr,oracle,params=None,ret=None):
        ...
        rows.append({'name':name,'source':f'fn {name}('+','.join(n+':'+t for n,t in pars)+f')->{ret or ty} {{return {expr};}}',
                     'params':pars,'ret':ret or ty,'oracle':oracle})
    def checked(v,ty):
        lo,hi=bounds(ty)
        return {'defined':True,'return':v} if lo<=v<=hi else {'defined':False}
    def division(x,y,ty,rem=False):
        if y==0 or (x==bounds(ty)[0] and y==-1):return {'defined':False}
        q=abs(x)//abs(y)
        if (x<0)!=(y<0):q=-q
        return checked(x-q*y if rem else q,ty)
    def wrapped(v,ty):return checked(v%(bounds(ty)[1]+1),ty)
    for ty in ['u8','u32','u64','i32','i64']:
        for op,label,fun in [('+','add',lambda x,y:x+y),('-','sub',lambda x,y:x-y),('*','mul',lambda x,y:x*y)]:
        ...
        if ty.startswith('u'):
            ...
            for expr,label,fun in [('shl_wrap(x,k)','left',lambda x,k:x<<k),('shr(x,k)','right',lambda x,k:x>>k)]:
                w=int(ty[1:])
                add(label+'_'+ty,ty,expr,lambda a,w=w,t=ty,f=fun:wrapped(f(a['x'],a['k']),t) if a['k']<w else {'defined':False},[('x',ty),('k','usize')])
        else:
...
def inputs(row,rng,exhaustive=True):
    ...
    axes=[]
    for n,t in pars:
        if t=='bool':vals=[False,True]
        else:
            lo,hi=bounds(t)
            vals=sorted(set(x for x in [lo,lo+1,-2,-1,0,1,2,7,8,31,32,63,64,127,128,255,256,hi//2,hi-1,hi] if lo<=x<=hi))
        axes.append(vals)
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--gcc',action='store_true');a=ap.parse_args()
    start=time.perf_counter();rows=fixtures();source='\n'.join(r['source'] for r in rows)
    (R/'results').mkdir(exist_ok=True);(R/'results/scalar_validation.cairn').write_text(source)
    refs=prepared(source);concrete=Concrete(refs);rng=random.Random(1709202604)
    cases={r['name']:inputs(r,rng) for r in rows};checks=[]
    with Solver(10000) as solver:
        for r in rows:
            name=r['name'];all_inputs=cases[name];selected=all_inputs if len(all_inputs)<150 else all_inputs[:30]+random.Random(42).sample(all_inputs,120)
            q=Formula(refs[name].params);sym=Symbolic(q,refs).invoke(name,list(q.inputs.values()))
            mismatches=[]
            for args in selected:
                expected=r['oracle'](args)
                equal_inputs=conj(*(same(q.inputs[n].value,('true' if args[n] else 'false') if t=='bool' else constant(args[n],t)) for n,t in r['params']))
                mismatch=neg(sym.defined) if expected['defined'] else sym.defined
                if expected['defined']:
                    v=('true' if expected['return'] else 'false') if r['ret']=='bool' else constant(expected['return'],r['ret'])
                    mismatch=disj(mismatch,conj(sym.defined,neg(same(sym.value,v))))
                mismatches.append(conj(equal_inputs,mismatch))
            smt=q.text(disj(*mismatches));result=solver.check(smt,q.variables)
            if result['status']!='unsat':raise AssertionError((name,result))
        ...
        with NativeScalar(source,cxx) as native:
            ...
            for r in rows:
                for args in cases[r['name']]:
                    expected=r['oracle'](args);actual=concrete.outcome(r['name'],args);machine=native.outcome(r['name'],args)
                    if not outcome_key(expected)==outcome_key(actual)==outcome_key(machine):raise AssertionError((r['name'],args,expected,actual,machine))
                    count+=1;traps+=not expected['defined']
            native_rows.append({'compiler':native.compiler,'cases':count,'trap_cases':traps,
                                'generated_sha256':native.generated_sha256,'instrumented_runtime_sha256':native.runtime_sha256})
    result={'status':'passed','seed':1709202604,'functions':len(rows),'native':native_rows,
            'smt_pinned_cases':sum(x['smt_pinned_cases'] for x in checks),'smt_checks':checks,'z3_version':version,

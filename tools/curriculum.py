...
def build():
    ...
    for family in ['affine','count_gt','prefix','compact_gt','clamp','interval','rotate_xor','delta']:
        ...
        for k in range(8):
            symbol=f'{family}_{k}'; cases=[]; A=k+2;B=k*3+1
            if family=='affine':
                ...
                for x in xs:
                    acc=0;out=[]
                    for v in x:acc=(acc+v)&MASK;out.append(acc)
                ...
                for x in xs:
                    selected=[v for v in x if v>B];out=selected+[123]*(len(x)-len(selected))
                    cases.append({'args':{'n':len(x),'out':[123]*len(x),'x':x,'threshold':B},'return':len(selected),'after':{'out':out}})
            elif family=='clamp':
                ...
                for x in xs:
                    for lo,hi in [(0,A),(A,A),(A+B,A),(A,A+B)]:
                        cases.append({'args':{'n':len(x),'x':x,'lo':lo,'hi':hi},'return':sum(lo<=v<hi for v in x)})
                ...
                for x in [0,1,MASK,2**63,*[rng.getrandbits(64) for _ in range(12)]]:
                    ...
                    cases.append({'args':{'x':x,'key':key},'return':(((x<<shift)&MASK)|(x>>(64-shift)))^key})
            else:
            ...
            tasks.append({'id':symbol,'family':family,'split':split,'source':text,
                'contract':{'schema':'cairn.task/1','symbol':symbol,'task':desc,
                    'allowed_effects':receipt['functions'][symbol]['effects'],'cases':cases}})
    return tasks

CONTRASTS=[
 ('mutability','fn f(x:u64)->u64{let y=x;y=add_wrap(y,1);return y;}','fn f(x:u64)->u64{let mut y=x;y=add_wrap(y,1);return y;}','E-IMMUTABLE'),
 ...
 ('view_extent_order','fn f(x:ro<u64>[n]@host,n:usize)->u64{return x[0];}','fn f(n:usize,x:ro<u64>[n]@host)->u64{return x[0];}','E-EXTENT'),
 ('no_readonly_store','fn f(n:usize,x:ro<u64>[n]@host)->u64{x[0]=1;return x[0];}','fn f(n:usize,x:ro<u64>[n]@host)->u64{return x[0];}','E-WRITE-LEASE'),
 ('no_float_narrow','fn f(x:f64)->u64{return u64(x);}','fn f(x:f64)->f64{return x;}','E-CAST'),
 ('no_alias','fn use(n:usize,o:rw<u64>[n]@host,x:ro<u64>[n]@host){}fn f(n:usize,o:rw<u64>[n]@host){use(n,o,o);}', 'fn use(n:usize,o:rw<u64>[n]@host,x:ro<u64>[n]@host){}fn f(n:usize,o:rw<u64>[n]@host,x:ro<u64>[n]@host){use(n,o,x);}','E-ALIAS'),
 ...
 ('collector_capacity','fn f(n:usize,m:usize,o:rw<u64>[n]@host)->usize{let used=compact o for i in m where true yield u64(i);return used;}','fn f(n:usize,m:usize,o:rw<u64>[n]@host)->usize{let used=compact o for i in n where true yield u64(i);return used;}','E-COLLECT-CAPACITY'),
]
...
def main():
    ...
    for family,bad,good,code in CONTRASTS:
        ...
        else:raise AssertionError('Negative teaching example compiled')
    for split in ['train','heldout']:
        rows=[]
        for t in tasks:
            if t['split']!=split:continue
            row={'id':t['id'],'family':t['family'],'task':t['contract']['task'],
                 'messages':[{'role':'system','content':'\n\n'.join(select_cards(t['source'],has_views='@host' in t['source']).values())},
                    {'role':'user','content':t['contract']['task']+'\nRequired signature: '+t['source'].split('{')[0].strip()}]}
            if split=='train':row['messages'].append({'role':'assistant','content':t['source']})
            rows.append(row)
    ...
    summary={'seed':17092603,'tasks':len(tasks),'train':sum(t['split']=='train' for t in tasks),
        'heldout':sum(t['split']=='heldout' for t in tasks),'contrastive_pairs':len(contrasts),
        ...
        'independent_families':8,'model_training_performed':False,'model_evaluations_performed':0,
        'native_validation':'run tools/curriculum_verify.py; generation checks frontend only',

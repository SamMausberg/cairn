...
The guard-elision rule of `compiler/facts.py` is Lean-checked in `proofs/Cairn/Facts.lean`: a discharged index is in bounds and a discharged `+`, `-`, shift or narrowing cannot trap, wherever the facts in scope hold. That the checker keeps only true facts in scope is tested, and a differential run ties the Python rule to the Lean one.

...
Host validation (verify/validation/validation.py), the device tests it generates (verify/validation/device.py), `cairn test`'s
...
def same(ty: str, reference: Any, candidate: Any, tolerance: dict[str, float]) -> bool:
    """Whether two results of type `ty` agree, as verify/validation/isolated_calls.py encodes them: a float as the hexadecimal
...
def helper(name: str, tolerance: dict[str, float]) -> str:
    ...
    lines = [f"fn {name}(r:f64, c:f64) -> bool {{  // {VERSION}, verify/validation/agreement.py"]

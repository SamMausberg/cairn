...
Measured, on single machines: at equal guards and worker counts a CAIRN parallel region runs level with OpenMP and oneTBB, and beats the guarded sequential loop from ten million elements on four kernels (`evidence/v1_3/bench/`). It lost where the 1.3 language kept a reduction or a shared-bin histogram sequential, as the preregistration predicted; `reduce op parallel` and `scan` now run on the lane pool, and the suite has not been rerun since. Device kernels run far ahead of the host, and transfers often cost more than the kernel (`evidence/v1_3/gpu/`). `cairn predict` was checked against host timings it was not fitted to: a median error of 28 to 44 percent, and a Kendall tau of 0.87 to 0.92 for how it ranks them (`evidence/v1_4/perf_model/`); its device side is a published specification no run has confirmed. Nothing is claimed against tuned C++ or CUDA.

...
- Every probe of a compiler's version allows two minutes. A cold `clang++ --version` on a loaded runner once took more than the five seconds the build allowed.

On the runs seen so far, the lint, examples and proofs jobs pass in about a minute each. The language-and-agent, tooling and remaining-tests parts pass in 3 to 5 minutes each. Most runs were cancelled by a newer push before the soundness part finished, so no run has yet been green from end to end in this record.
...
Compiling every example project, every template and a program importing all of std, 19 programs in one process, took a median of 2,378 ms before and 2,001 ms after: 0.84 of the time, over 8 interleaved rounds of three passes each, with rounds from 1,788 to 2,664 ms. As a fresh process, `cairn check` gains less, since the parse cache lives for one process. Over 10 interleaved runs, `examples/hello` went from 172 to 157 ms, and analytics, panel and wordfreq moved by -4% to +4%, within the noise.

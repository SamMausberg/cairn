...
def validate_private(record: dict, owner: str, name: str) -> None:
    if (not isinstance(record,dict) or record.get('private') is not True
...
def publish(root: Path, repository: str, *, execute: bool = False,
            run=command, audit_fn=audit) -> dict:
    ...
    root=root.resolve()
    def git(*args):return run(['git',*args],cwd=root)
    def gh(*args):return run(['gh','api','--hostname','github.com',*args],cwd=root)
    ...
    if not re.fullmatch(r'[0-9a-f]{40}',head):raise PublishError('Invalid commit identity.')
    git('fsck','--strict')
    scan=audit_fn(root)
    if scan['findings']:raise PublishError('Tracked-history scan blocked publication. Inspect audit_repository.py output.')
    if not execute:
        return {'status':'local-preflight-passed','remote_created':False,'repository':repository,
                'head':head,'visibility_required':'private','audit':scan,
    ...
    created=json.loads(gh('--method','POST','user/repos','-f','name='+name,
                         '-F','private=true','-F','auto_init=false','-F','has_wiki=false',
                         '-F','has_projects=false', '-f','description=CAIRN native language development'))
    validate_private(created,owner,name)
    endpoint='repos/'+repository
    confirmed=json.loads(gh(endpoint));validate_private(confirmed,owner,name)
    ...
    git('-c','credential.helper=', '-c','credential.helper=!gh auth git-credential',
        '-c','core.hooksPath=/dev/null','push',url,head+':refs/heads/main')
    final=json.loads(gh(endpoint));validate_private(final,owner,name)
    if final.get('id')!=repo_id:raise PublishError('Final repository identity mismatch.')
    ...
    return {'status':'published-private','repository':repository,'head':head,'repository_id':repo_id,
            'private':True,'url':final.get('html_url'), 'force_push':False,
...
def main() -> int:
    ...
    p.add_argument('--execute', action='store_true', help='Actually create and push; default performs local checks only.')
    a=p.parse_args()
    try:
        print(json.dumps(publish(Path(__file__).resolve().parents[1],a.repository,execute=a.execute),indent=2));return 0
    except (OSError,ValueError,PublishError,subprocess.SubprocessError) as error:
        print(json.dumps({'status':'stopped','message':str(error),
                          'policy':'No retries, repository deletion, force push or public fallback.'}));return 1

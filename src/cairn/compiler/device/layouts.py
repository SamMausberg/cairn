...
from ..syntax.tree import USIZE, Expr, Type, fail
...
LARGEST = 2**63 - 1  # the most elements an array holds (check/checking.py), so every offset a layout places is below
...
def natural(c: Checker, e: Expr, least: int = 1) -> int:
    from ..check.constants import fold  # constants.py folds layout queries through this module, so it is imported here
...
def apply(c: Checker, e: Expr, given: tuple[Any, ...]) -> int | None:
    ...
    for a rule that runs a body with numbers (compiler/cooperative/block_run.py): None when an argument is not a number, and

...
TASK = {
    "symbol": "average",
    "task": "For all u64 inputs, return floor((x+y)/2) over mathematical integers without overflow or traps.",
}
...
def main():
    ...
    (folder / "before.cairn").write_text(SOURCE)
    ...
    for cache in [False, True]:
        ...
        r["counterexample_cache"] = cache
        outputs.append(r)
    candidate = outputs[-1]["candidate"]
    (folder / "after.cairn").write_text(candidate)
    (folder / "choices.json").write_text(json.dumps(outputs[-1]["choices"], indent=2) + "\n")
    queries = []
    certificate = sk.check_semantics(sk.fill(**outputs[-1]["choices"]), query_log=queries)
    ...
    density = {
        ...
        "old_expression_packet": tokens(oldpacket),

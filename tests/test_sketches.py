...
def sketch():
    return Sketch(BEFORE,'f',task={'task':'Floor average; no traps on valid u64 inputs.'},semantic=ScalarContract(REF,'f')).hole('average','(x+y)/2')

def test_fill_preserves_surroundings():
    s=sketch();r=s.fill(average='(x&y)+shr(x^y,1)')
...
@pytest.mark.parametrize('values',[{}, {'extra':'x'}, {'average':'x','extra':'y'}, {'average':12}, {'average':'x; return 7;'}, {'average':'unknown'}, {'average':'true'}])
def test_reject_bad_choices(values):
    with pytest.raises(Diagnostic):sketch().fill(**values)

def test_nested_slots_rejected():
    s=sketch()
    with pytest.raises(Diagnostic):s.hole('input_x','x',occurrence=0)

def test_repeated_expression_needs_explicit_occurrence():
    src='fn f(x:u64)->u64{return add_wrap(x,x);}'
    s=Sketch(src,'f')
    with pytest.raises(Diagnostic):s.hole('one','x')
    s.hole('one','x',occurrence=0).hole('two','x',occurrence=1)
    r=s.fill(one='1',two='2');compile_source(r.source)

def test_no_post_packet_hole_change():
    s=sketch();s.packet()
    with pytest.raises(Diagnostic):s.hole('xslot','x')

def test_changed_candidate_rejected():
    s=sketch();r=s.fill(average='x');r.source=r.source.replace('(x)','(y)')
    with pytest.raises(Diagnostic):s.check_semantics(r)
...
def test_changed_host_task_rejected():
    s=sketch();s.packet();s._session.contract['task']='different'
    with pytest.raises(Diagnostic):s.fill(average='x')
...
def test_search_dimension_errors(choices):
    with pytest.raises(Diagnostic):solve_finite(sketch(),choices)
...
def test_strict_choice_transport_rejects(bad):
    ...
    with pytest.raises(Diagnostic):sk.fill_json(bad)

...
bench/scan/scan.cairn does the timing inside one process, interleaved; this builds it the way `cairn build`
...
def measure(cxx: str, reps: int, work: Path) -> dict:
    source = (ROOT / "bench/scan/scan.cairn").read_text().replace("REPS:usize = 15", f"REPS:usize = {reps}")

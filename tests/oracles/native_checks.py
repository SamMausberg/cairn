...
class Header(C.Structure):
    _fields_ = [("version", U32), ("flags", U32), ("length", U64)]
...
def load(path=None):
    ...
    for name, (args, ret) in SIGS.items():
        ...
        f.argtypes = args
        f.restype = ret
    lib.cf_translated.argtypes = [Vec3, Vec3]
    lib.cf_translated.restype = Vec3
    return lib
...
def trap_case(name):
    hard = resource.getrlimit(resource.RLIMIT_CORE)[1]  # no core: 1 where allowed, since a pipe takes one under 0
...
def run(libpath=None):
    ...
    for case in range(180):
        ...
        assert list(of) == [f32(f32(a * q) + v) for q, v in zip(x, y)]
        count["saxpy"] += 1
        expected = 0.0
        for q, v in zip(x, y):
            expected = expected + q * v
        assert bits(l.cf_dot(n, xd, yd)) == bits(expected)
        count["dot"] += 1
        assert l.cf_sum_wrap(n, ux) == sum(u) & MASK
        count["sum_wrap"] += 1
        expected = []
        running = 0
        for q in u:
            running = (running + q) & MASK
            expected.append(running)
        l.cf_prefix(n, ou, ux)
        assert list(ou) == expected
        count["prefix"] += 1
        assert l.cf_count_gt(n, xf, a) == sum(q > a for q in x)
        count["count_gt"] += 1
        hist_in = [r.randrange(256) for _ in range(n)]
        hi = array(U8, hist_in)
        ho = array(U64, [99] * 256)
        l.cf_histogram(n, ho, hi)
        exp = [0] * 256
        for q in hist_in:
            exp[q] += 1
        assert list(ho) == exp
        count["histogram"] += 1
        got = l.cf_compact_even(n, ou, ux)
        exp = [q for q in u if q % 2 == 0]
        assert got == len(exp) and list(ou)[:got] == exp
        count["compact_even"] += 1
        ordered = sorted(u)
        key = r.getrandbits(64)
        so = array(U64, ordered)
        assert l.cf_lower_bound(n, so, key) == bisect.bisect_left(ordered, key)
        count["lower_bound"] += 1
        l.cf_copy_twice(n, of, xf)
        assert list(of) == [f32(2 * q) for q in x]
        count["copy_twice"] += 1
        l.cf_delegated(n, of, xf)
        assert list(of) == [f32(2 * q) for q in x]
        count["delegated"] += 1
        sq = 0.0
        for q in x:
            sq = sq + q * q
        assert bits(l.cf_square_sum(n, xd)) == bits(sq)
        count["square_sum"] += 1
        if n:
            ix = r.randrange(n)
            assert l.cf_checked_at(n, ux, ix) == u[ix]
            count["checked_at"] += 1
        av = r.getrandbits(64)
        bv = r.getrandbits(64)
        assert l.cf_gcd(av, bv) == math.gcd(av, bv)
        count["gcd"] += 1
        f = r.randrange(0, 100)
        assert l.cf_factorial_wrap(f) == math.factorial(f) & MASK
        count["factorial_wrap"] += 1
        aa = [r.randrange(-100, 100) / 4 for _ in range(3)]
        bb = [r.randrange(-100, 100) / 4 for _ in range(3)]
        d = [a - b for a, b in zip(aa, bb)]
        val = d[0] * d[0] + d[1] * d[1] + d[2] * d[2]
        assert bits(l.cf_distance_sq(Vec3(*aa), Vec3(*bb))) == bits(val)
        count["distance_sq"] += 1
        tr = l.cf_translated(Vec3(*aa), Vec3(*bb))
        assert (tr.x, tr.y, tr.z) == tuple(a + b for a, b in zip(aa, bb))
        count["translated"] += 1
        ...
        pa = array(U8, payload)
        off = r.randrange(29)
        assert l.cf_decode_le32(32, pa, off) == int.from_bytes(payload[off : off + 4], "little")
        count["decode_le32"] += 1
        av = r.randrange(1 << 60)
        bv = r.randrange(1 << 60)
        assert l.cf_checked_add(av, bv) == av + bv
        count["checked_add"] += 1
        av = r.randrange(-(1 << 60), 1 << 60)
        bv = r.choice([-1, 1]) * r.randrange(1, 1 << 30)
        exp = abs(av) // abs(bv)
        exp = -exp if (av < 0) != (bv < 0) else exp
        assert l.cf_checked_div(av, bv) == exp
        count["checked_div"] += 1
        av = r.randrange(1 << 32)
        shift = r.randrange(32)
        assert l.cf_checked_shift(av, shift) == (av << shift) & 0xFFFFFFFF
        count["checked_shift"] += 1
        av = r.randrange(256)
        assert l.cf_narrow(av) == av
        count["narrow"] += 1
    # Empty views may be null. Boundary checker must not dereference them.
    l.cf_saxpy(0, None, None, None, 1)
    ...
    for k in range(1, 257):
        ...
        for n in [0, 1, 7, 31, 257]:
            vals = [r.randrange(-256, 257) / 8 for _ in range(n)]
            a = array(F32, vals)
            b = array(F32, [0] * n)
            fn(n, b, a)
            assert list(b) == [f32(v * k) for v in vals]
            count["family_specialization"] += 1
    traps = [
        "overflow",
        "division_zero",
        "division_overflow",
    ...
    return {
        "seed": 20260917,
        "function_cases": dict(count),
        "total_function_cases": sum(count.values()),
        "expected_abort_cases": len(traps),
        "all_passed": True,

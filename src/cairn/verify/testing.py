...
def scalar(name, value):
    if name not in CTYPES:
        raise ValueError("Task runner supports scalar/view signatures only.")
    if name == "bool":
        if type(value) is not bool:
            raise ValueError("Boolean argument must be JSON boolean.")
    elif name in LIMITS:
        lo, hi = LIMITS[name]
...
def validate_contract(source, contract):
    if not isinstance(contract, dict) or contract.get("schema") != "cairn.task/1":
        raise ValueError("Unsupported task schema.")
    fs = {f.name: f for f in Parser(source).parse().functions}
    name = contract.get("symbol")
    if name not in fs or fs[name].static:
        raise ValueError("Contract needs an ordinary authored symbol.")
    f = fs[name]
    if f.ret.name not in CTYPES and f.ret.name != "void":
        raise ValueError("Task runner does not support record returns.")
    cases = contract.get("cases")
    if not isinstance(cases, list) or not 1 <= len(cases) <= 10000:
        raise ValueError("Need 1..10000 trusted cases.")
    for case in cases:
        if not isinstance(case, dict) or set(case) - {"args", "return", "after"}:
            raise ValueError("Invalid case keys.")
        args = case.get("args")
        if not isinstance(args, dict) or set(args) != {n for n, _ in f.params}:
            raise ValueError("Argument names do not match signature.")
        for n, t in f.params:
            if t.mode == "value":
                scalar(t.name, args[n])
            else:
                size = int(t.extent) if t.extent.isdigit() else args[t.extent]
                if type(size) is not int or not 0 <= size <= 100000:
                    raise ValueError("View size exceeds runner limit.")
                if not isinstance(args[n], list) or len(args[n]) != size:
                    raise ValueError("View size mismatch.")
                for value in args[n]:
                    scalar(t.name, value)
        if f.ret.name == "void":
            if case.get("return") is not None:
                raise ValueError("Void return must be null or absent.")
        else:
            if "return" not in case:
                raise ValueError("Every scalar return needs an expected value.")
            scalar(f.ret.name, case["return"])
        after = case.get("after", {})
        writable = {n: t for n, t in f.params if t.mode == "rw"}
        if set(after) != set(writable):
            raise ValueError("Every mutable output needs a full expected post-state.")
        for n, t in writable.items():
            if not isinstance(after[n], list) or len(after[n]) != len(args[n]):
                raise ValueError("Post-state size mismatch.")
            for value in after[n]:
                scalar(t.name, value)
    return f
...
def child(library, source, contract):
    ...
    for i, case in enumerate(contract["cases"]):
        ...
        for n, t in f.params:
            value = case["args"][n]
            if t.mode == "value":
                args.append(CTYPES[t.name](value))
            else:
                array = (CTYPES[t.name] * len(value))(*value)
                storage[n] = array
                args.append(array)
        actual = native(*args)
...
def evaluate(source: str, contract: dict, cxx="clang++", libraries: tuple[str, ...] = ()) -> dict:
    ...
    try:
        ...
        return {**common, **explain(e, source), "stage": "frontend"}
    except (ValueError, TypeError, KeyError, OverflowError) as e:
        return {**common, "status": "invalid-contract", "message": str(e)}
...
def main():
    ...
    return 0 if result["status"] == "passed-finite-tests" else 1

...
array where either writes (compiler/cooperative/phases.py: E-COOP-CONFLICT, E-COOP-UNORDERED, E-COOP-REUSE, E-COOP-UNDECIDED),
and every element of an array from outside is written by at most one thread of one block, and read by another only
if nobody writes it (compiler/cooperative/footprints.py: E-COOP-GLOBAL). Everything else a lane may not do, a thread may not do.
...
from ..check.constants import constant
from ..check.effects import DEVICE_SAFE, LANE_SAFE
from ..check.scope import Binding, Block, Lanes
from ..device import fragments
...
from . import block_run, finish, footprints, phases, pipelines
...
ALIGN = 128  # where each shared array starts: tensor-core fragments load from 32 bytes on (device/fragments.py)
...
class Reach:
    ...
    def lends(self, e: Expr, at: tuple[int, Any]):
        ...
        receiver."""
        from ..primitives.builtins import TABLE
...
def s_blocks(c: Checker, s: Stmt, main: Block | None = None):
    """A region, or with `main` the finish of the region `main` (compiler/cooperative/finish.py), which runs where it runs."""
...
def check_shuffle(c: Checker, e: Expr, args: list[Expr], targs: tuple, expected: Type | None) -> Type:
    ...
    from ..primitives.builtins import arity
...
def lower_blocks(g: Emitter, s: Stmt, es: list[str]):
    ...
    def whole():
        ...
        if s.other:  # the region's finish, after every block (compiler/cooperative/finish.py)

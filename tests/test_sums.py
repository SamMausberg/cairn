...
import subprocess
import pytest
from cairn.cairnc import compile_source,Diagnostic,RUNTIME
from cairn.agent_tools import canonical_source,EditSession
...
@pytest.mark.parametrize('decl,body,code',[
 ...
 ('enum R { Good(u64); Bad; }','match R.Bad {R.Bad=>{return R.Bad;} R.Bad=>{return R.Bad;}}','E-MATCH-DUPLICATE'),
 ('enum R { Good(u64); Bad; }','match R.Bad {R.Good=>{return R.Bad;} R.Bad=>{return R.Bad;}}','E-MATCH-BINDING'),
 ('enum R { Good(u64); Bad; }','match R.Bad {R.Good(v)=>{return R.Good(v);} R.Bad(v)=>{return R.Bad;}}','E-MATCH-BINDING'),
 ('enum R { Good(u64); Bad; }','let v:u64=0;match R.Bad {R.Good(v)=>{return R.Good(v);} R.Bad=>{return R.Bad;}}','E-SHADOW'),
 ('enum R { Good(u64); Bad; }','match R.Bad {R.Good(v)=>{} R.Bad=>{}} return R.Good(v);','E-UNBOUND'),
 ('enum R { Good(u64); Bad; }','match 1 {R.Good(v)=>{return R.Good(v);} R.Bad=>{return R.Bad;}}','E-MATCH-TYPE'),
 ...
 ('enum R { Good(u64); Bad; }','match R.Bad {R.Good(v)=>{v=2;return R.Good(v);} R.Bad=>{return R.Bad;}}','E-IMMUTABLE'),
])
def test_reject(decl,body,code):
    with pytest.raises(Diagnostic) as e:compile_source(decl+'fn f()->R {'+body+'}')
    assert e.value.data['code']==code

@pytest.mark.parametrize('decl',[
 'enum R { Bad(void); }', 'enum R { Bad(ro<u64>[1]); }',
 'enum R { Bad(R); }', 'struct S {x:u64;} enum R {Bad(S);}',
])
def test_payload_restrictions(decl):
    with pytest.raises(Diagnostic) as e: compile_source(decl)
    assert e.value.data['code']=='E-SUM-PAYLOAD'

def test_sum_view_rejected():
    with pytest.raises(Diagnostic) as e:compile_source('enum R {V(u64);} fn f(n:usize,x:ro<R>[n]){}')
...
def test_nested_writes_in_arm_rejected():
    ...
    with pytest.raises(Diagnostic) as e:compile_source(src)
    assert e.value.data['code']=='E-EFFECT-ORDER'

@pytest.mark.parametrize('cxx',['clang++','g++'])
def test_native(cxx,tmp_path):
    cpp,_=compile_source(SOURCE)
    (tmp_path/'p.cpp').write_text(cpp);(tmp_path/'cairn_runtime.hpp').write_text(RUNTIME)
    subprocess.run([cxx,'-std=c++20','-O2','-fno-exceptions','-fno-rtti','-Werror','-shared','-fPIC',str(tmp_path/'p.cpp'),'-o',str(tmp_path/'p.so')],check=True,capture_output=True)
    lib=ctypes.CDLL(str(tmp_path/'p.so'))
    lib.cf_use.argtypes=[ctypes.c_uint64]*2;lib.cf_use.restype=ctypes.c_uint64
    lib.cf_assign.argtypes=[ctypes.c_uint64];lib.cf_assign.restype=ctypes.c_uint64
    for a in [0,1,7,2**32,2**64-1]:
        assert lib.cf_assign(a)==a
        for b in [0,1,2,7,2**64-1]: assert lib.cf_use(a,b)==(a//b if b else 42)
    lib.cf_decode.argtypes=[ctypes.c_uint8];lib.cf_decode.restype=ctypes.c_uint64
    for c in range(256):assert lib.cf_decode(c)==(c-48 if 48<=c<=57 else c+1000)
    lib.cf_scoped.argtypes=[ctypes.c_size_t];lib.cf_scoped.restype=ctypes.c_uint64
    for n in [0,1,2,16]:assert lib.cf_scoped(n)==0

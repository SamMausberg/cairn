...
A body edit (`edits.py`) rewrites a function inside its signature; a plan edit (`plans.py`) changes only a
...
the reference on boundary inputs (verify/validation/validation.py). Only a validated implementation advances the source, and no
...
from ...verify.validation.validation import EMULATED, FINITE, Policy, failure, held, refusal, validate
...
def pinned(declaration: str, policy: dict[str, Any]) -> dict[str, str]:
    ...
    the numerical policy (verify/validation/agreement.py) that compares float results."""
    return {

...
def select_cards(source: str, has_views: bool=False, has_records: bool=False,
                 has_sums: bool=False) -> dict[str,str]:
    ...
    if has_views or words & {'buffer','stack','len'}:selected.append('views')
    if 'compact' in words:selected.append('compact')
    if words & {'f32','f64'}:selected.append('floats')
    if has_records:selected.append('records')
    if words & {'family','derive'}:selected.append('generators')
    if words & {'buffer','stack'}:selected.append('memory')
    if has_sums or 'match' in words:selected.append('sums')

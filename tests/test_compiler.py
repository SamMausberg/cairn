...
BAD = [
 ...
 ('E-CALL-SHAPE','fn f(n:usize,x:ro<u64>[n]@host)->u64{return 0;} fn g(n:usize,x:ro<u64>[n]@host)->u64{return f(n+1,x);}'),
 ('E-TYPE-MISMATCH','fn f(n:usize,x:ro<u64>[n]@host)->u64{return 0;} fn g(n:usize,m:usize,x:ro<u64>[n]@host)->u64{return f(m,x);}'),
 ('E-ALIAS','fn f(n:usize,a:rw<u64>[n]@host,b:ro<u64>[n]@host){} fn g(n:usize,a:rw<u64>[n]@host){f(n,a,a);}'),
 ('E-EFFECT-ORDER','fn f(n:usize,a:rw<u64>[n]@host)->u64{a[0]=1;return 0;} fn g(n:usize,a:rw<u64>[n]@host)->u64{return 1+f(n,a);}'),
 ('E-LEX','fn f() { $; }'),
 ('E-MINMAX','fn f(x:f64)->f64{return min(x,x);}'),
]
@pytest.mark.parametrize('code,source',BAD)
def test_rejections(code,source):
    with pytest.raises(Diagnostic) as e: compile_source(source)
    assert e.value.data['code']==code

def test_native_examples():
    root=Path(__file__).resolve().parents[1]
    _,r=compile_source((root/'examples/native.cairn').read_text())
...
def test_whole_expression_effect_call():
    compile_source('fn f(n:usize,a:rw<u64>[n]@host)->u64{a[0]=1;return 0;} fn g(n:usize,a:rw<u64>[n]@host)->u64{let q=f(n,a); return q;}')

MORE_BAD=[
 ('E-COLLECT-CAPACITY','fn f(n:usize,m:usize,o:rw<u64>[n]@host,x:ro<u64>[m]@host)->usize{let k=compact o for i in m where true yield x[i];return k;}'),
 ('E-COLLECT-SELF-READ','fn f(n:usize,o:rw<u64>[n]@host)->usize{let k=compact o for i in n where true yield o[i];return k;}'),
 ('E-WRITE-LEASE','fn f(n:usize,o:ro<u64>[n]@host)->usize{let k=compact o for i in n where true yield 0;return k;}'),
 ('E-COLLECT-BINDING','fn f(n:usize,o:rw<u64>[n]@host)->usize{reg k=compact o for i in n where true yield 0;return k;}'),
 ('E-EFFECT-ORDER','fn w(n:usize,x:rw<u64>[n]@host)->u64{x[0]=1;return 0;} fn f(n:usize,o:rw<u64>[n]@host,x:rw<u64>[n]@host)->usize{let k=compact o for i in n where true yield w(n,x);return k;}'),
...
def test_collector_receipt():
    _,r=compile_source('fn f(n:usize,o:rw<u64>[n]@host)->usize{let k=compact o for i in n where true yield u64(i);return k;}')

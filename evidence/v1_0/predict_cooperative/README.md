...
Taken on 2026-09-23 at 9a8b299 on one AMD Ryzen 7 7800X3D (8 cores, 16 threads) under WSL2, with CUDA 13.2 (nvcc V13.2.78), by `python bench/predict/cooperative.py --out evidence/v1_0/predict_cooperative`. Other agents were building and testing on the machine: the load average was 26 on 16 threads, so the one wall time here, `tune.json`'s `seconds`, is a loaded machine's.

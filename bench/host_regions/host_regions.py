#!/usr/bin/env python3
"""Compile and run bench/host_regions/host_regions.cpp and record the result with its environment.
...
def measure(cxx: str, runtime: Path, arch: str, build: Path) -> dict:
    ...
    line = [
        cxx,
        *profile_flags("exe", arch),
        f"-I{runtime}",
        str(ROOT / "bench/host_regions/host_regions.cpp"),
        "-o",
        str(exe),
    ]

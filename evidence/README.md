...
| `v1_5/` | Records of the work after 1.4, taken as it landed, one directory per subject with a README saying what ran and what did not: `demos/` for the three demos, `perf/` for the compiler, the test suite, CI and the lane pool, `trim/` for the pass that shared duplicated logic in `src/cairn`, `scale/` for check, build, rebuild and editor times of generated projects up to 77,000 lines and the size limits, and `ai_benchmark/` for the preregistered equal-budget benchmark of CAIRN, C++ and Rust, whose `RESULTS.md` says what ran and what did not. |

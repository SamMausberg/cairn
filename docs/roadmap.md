...
- The kernel features [devices.md](devices.md#what-fast-kernels-use) marks foreign or typed PTX: warp match and `shuffle_up`, dynamic shared memory past 48 KiB, packed half and bf16 math, fast approximate math with stated error bounds, telling nvcc what an `ro` view promises, `wgmma`, TMA, clusters, grid-wide sync and CUDA graphs. Until one lands, a kernel that needs it is a foreign CUDA implementation that `cairn foreign` inspects and `cairn validate` holds to its reference.

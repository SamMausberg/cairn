...
def rejected(code,s,r):
    with pytest.raises(Diagnostic) as e:s.check(r)
...
def test_projection(path):
    original=(R/path).read_text(); view=canonical_source(original)
...
def test_exact_outside_preservation():
    s=EditSession(S,'step');f=s.f
    new,r=s.check(request(s,'{return mul_wrap(x,3);}'))
    assert new[:f.body_start]==S[:f.body_start]
    assert new.endswith(S[f.end:]); assert r['status']=='typed'
    assert r['behavioral_tests']=='not-run'

@pytest.mark.parametrize('body',[
 '{return x;} fn injected()->u64{return 0;}',
 '{return x;} //comment\n fn injected()->u64{return 0;}',
 'return x;', '{return x;', '{return true;}', '{return missing;}',
 '{return x;} ;', '{return x;} #include <stdio.h>'
])
def test_bad_bodies(body):
    s=EditSession(S,'step')
    with pytest.raises(Diagnostic):s.check(request(s,body))

def test_stale_session():
    s=EditSession(S,'step');r=request(s,'{return x;}');r['session']='0'*64
    rejected('E-SESSION',s,r)
    other=EditSession(S+'\n','step')
    rejected('E-SESSION',other,request(s,'{return x;}'))

def test_policy_session_binding():
    s=EditSession(S,'step',{'task':'one'});q=EditSession(S,'step',{'task':'two'})
    rejected('E-SESSION',q,request(s,'{return x;}'))

def test_request_unknown_key():
    s=EditSession(S,'step');r=request(s,'{return x;}');r['allowed_effects']=['trap']
    rejected('E-REQUEST',s,r)

def test_duplicate_json():
    with pytest.raises(Diagnostic):load_json_strict('{"kind":"body","kind":"expr"}')

def test_effect_ceiling():
    s=EditSession(S,'step');rejected('E-EFFECT-EXPANSION',s,request(s,'{return x+1;}'))

def test_hidden_callee():
    s=EditSession(S,'step');rejected('E-CONTEXT-CLOSURE',s,request(s,'{return other(x);}'))
...
def test_body_keeps_parameters_immutable():
    s=EditSession(S,'step');rejected('E-IMMUTABLE',s,request(s,'{x=1;return x;}'))
...
def test_expression_expected_type():
    ...
    try:s.check(req)
        ...
        d=explain(e);assert d['expected_type']=='u64' and d['actual_type']=='bool'
    else:assert False
...
def test_expression_injection(payload):
    s=EditSession(S,'step')
    with pytest.raises(Diagnostic):s.check(expression(s,'x',payload))
...
def test_lexical_scope():
    ...
    for site in s.sites.values():
        if site['source']=='add_wrap(total,y)':assert set(site['bindings'])=={'n','x','total','i','y'}
...
def test_generated_dependency_disclosed():
    ...
    s=EditSession(source,'top');text=json.dumps(s.packet())
    assert 'family scale = f[1..3];' in text and 'fn f[K:nat]' in text

def test_unrelated_template_not_editable():
    with pytest.raises(Diagnostic):EditSession('fn f[K:nat]()->usize{return K;}family x=f[1..3];','f')

def test_json_nonfinite():
    with pytest.raises(Diagnostic):load_json_strict('{"n":NaN}')

def test_candidate_filter():
    s=EditSession(S,'step');k=next(k for k,v in s.sites.items() if v['source']=='add_wrap(x,1)')

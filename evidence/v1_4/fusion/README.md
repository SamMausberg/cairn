...
|---|---|---|---|---|---|---|
| blend | clang++ | 1e5 | 0.067 | 0.038 | 1.77x | 2.89x |
| blend | clang++ | 1e6 | 0.447 | 0.114 | 3.93x | 4.07x |
| blend | clang++ | 1e7 | 65.485 | 3.305 | 19.81x | 27.14x |
| blend | clang++ | 3e7 | 213.316 | 13.986 | 15.25x | 16.59x |
| layers | clang++ | 1e5 | 0.082 | 0.032 | 2.56x | 2.33x |
| layers | clang++ | 1e6 | 0.398 | 0.283 | 1.41x | 1.97x |
| layers | clang++ | 1e7 | 14.672 | 10.107 | 1.45x | 1.31x |
| layers | clang++ | 3e7 | 52.036 | 33.658 | 1.55x | 1.53x |
| energy | clang++ | 1e5 | 0.104 | 0.068 | 1.53x | 1.43x |
| energy | clang++ | 1e6 | 1.119 | 0.694 | 1.61x | 1.28x |
| energy | clang++ | 1e7 | 64.878 | 7.153 | 9.07x | 10.09x |
| energy | clang++ | 3e7 | 180.977 | 22.821 | 7.93x | 10.98x |
| digest | clang++ | 1e5 | 0.067 | 0.031 | 2.19x | 3.20x |
| digest | clang++ | 1e6 | 0.413 | 0.120 | 3.44x | 5.73x |
| digest | clang++ | 1e7 | 60.782 | 0.979 | 62.06x | 203.75x |
| digest | clang++ | 3e7 | 195.869 | 5.522 | 35.47x | 55.03x |
| blend | g++ | 1e5 | 0.061 | 0.023 | 2.66x | 2.89x |
| blend | g++ | 1e6 | 0.504 | 0.151 | 3.34x | 4.07x |
| blend | g++ | 1e7 | 68.129 | 5.067 | 13.45x | 27.14x |
| blend | g++ | 3e7 | 212.905 | 15.298 | 13.92x | 16.59x |
| layers | g++ | 1e5 | 0.052 | 0.048 | 1.08x | 2.33x |
| layers | g++ | 1e6 | 0.492 | 0.228 | 2.16x | 1.97x |
| layers | g++ | 1e7 | 15.277 | 10.533 | 1.45x | 1.31x |
| layers | g++ | 3e7 | 57.871 | 34.730 | 1.67x | 1.53x |
| energy | g++ | 1e5 | 0.137 | 0.070 | 1.97x | 1.43x |
| energy | g++ | 1e6 | 1.583 | 0.723 | 2.19x | 1.28x |
| energy | g++ | 1e7 | 77.853 | 7.498 | 10.38x | 10.09x |
| energy | g++ | 3e7 | 229.662 | 22.648 | 10.14x | 10.98x |
| digest | g++ | 1e5 | 0.103 | 0.046 | 2.25x | 3.20x |
| digest | g++ | 1e6 | 0.491 | 0.318 | 1.54x | 5.73x |
| digest | g++ | 1e7 | 63.015 | 1.623 | 38.82x | 203.75x |
...
`layers` isolates what fusion itself saves, since it keeps every array: one pass over the extent instead of three, reading back what the earlier bodies just wrote. It ran 1.08x to 2.56x faster under both compilers, and the model predicts 1.31x to 2.33x.

The first model was wrong where it mattered most for `cairn tune` in two places. It predicted that fusing `energy` below ten million elements loses, 0.73x to 0.81x, because the fused squares then run on the fold's one thread; they ran 1.5x to 2.2x faster, because the fold is bound by the latency of its dependent adds and the independent squares hide in that latency, while the model added the costs of the two. The second pass prices a fold as the larger of its chain and the rest of its body, and now predicts 1.28x to 1.43x there, the right side of one. It still overestimates `digest` fused at ten million elements, 204x against 39x to 62x, since an 80 MiB stream on every lane ran at nearer memory's rate than the last cache's; that one is open, and a prediction there says so.

...
from .cairnc import (Parser, Checker, Emitter, Program, Function, Expr, Stmt, Type,
                    specialize, derive_wire, compile_source, Diagnostic, fail, VERSION)
...
def format_expr(e: Expr) -> str:
    if e.tag in {'name','int','float','bool'}: return e.val
    if e.tag == 'call': return e.val + '(' + ', '.join(map(format_expr,e.args)) + ')'
    if e.tag == 'index': return format_expr(e.args[0])+'['+format_expr(e.args[1])+']'
    if e.tag == 'field': return format_expr(e.args[0])+'.'+e.val
    if e.tag == 'unary': return '('+e.val+format_expr(e.args[0])+')'
    if e.tag == 'binary': return '('+format_expr(e.args[0])+' '+e.val+' '+format_expr(e.args[1])+')'
...
def format_block(ss: list[Stmt], indent: int = 0) -> str:
    ...
    for s in ss:
        ...
        else: fail('E-PROJECTION','Cannot project unknown statement kind.')
    lines.append('  '*indent+'}')
    return '\n'.join(lines)
...
def semantic_ast(p: Program) -> Any:
    ...
    def erase(v):
        if isinstance(v,dict):
            return {k:erase(x) for k,x in v.items() if k not in
                    {'line','col','start','end','body_start','cpp','ty_inferred'}}
        if isinstance(v,list):return [erase(x) for x in v]
        if isinstance(v,tuple):return [erase(x) for x in v]
        return v
...
def load_json_strict(text: str) -> Any:
    def pairs(items):
        d={}
        for k,v in items:
            if k in d: fail('E-REQUEST','Duplicate JSON field: '+k)
            d[k]=v
        return d
    try:
        return json.loads(text,object_pairs_hook=pairs,parse_constant=lambda s:fail('E-REQUEST','Nonfinite JSON value'))
    except (json.JSONDecodeError, RecursionError) as e:
        fail('E-REQUEST',str(e))

class EditSession:
    def __init__(self, source: str, symbol: str, contract: dict[str,Any] | None = None,
                 include: tuple[str,...] = ()):
        self.source=source
        self.symbol=symbol
        self.contract={} if contract is None else copy.deepcopy(contract)
        if not isinstance(self.contract,dict):fail('E-CONTRACT','Contract must be a host-owned JSON object.')
        if self.contract.get('symbol',symbol) != symbol:fail('E-CONTRACT','Contract names another function.')
        self.parsed=Parser(source).parse()
        authored={f.name:f for f in self.parsed.functions}
        if symbol not in authored:fail('E-SYMBOL','Edit an authored function, not a generated entry.')
        self.f=authored[symbol]
        if self.f.static:fail('E-EDIT-PROFILE','Template-body editing is not implemented; edit ordinary functions.')
        self.cpp,self.receipt=compile_source(source)
        p=specialize(derive_wire(Parser(source).parse()))
        checker=Checker(p,capture_sites=True); checker.check()
        self.expanded=p
        ...
        fs=self.receipt['functions']; selected={symbol,*include}
        if selected-set(fs):fail('E-SYMBOL','An included symbol is not in this module.')
        changed=True
        while changed:
            old=set(selected)
            for n,r in fs.items():
                if n in selected or set(r['calls'])&selected:
                    selected.add(n); selected.update(r['calls'])
            changed=old != selected
        self.visible=selected
        self.sites={}
        for site in checker.sites:
            if site['symbol']!=symbol or site['end']<=site['start']:continue
            ...
            self.sites[key]={**site,'site':key,
                'source':source[site['start']:site['end']]}
        from pathlib import Path
        files=sorted(p for p in Path(__file__).parent.rglob('*') if p.suffix in {'.py','.hpp'} and '__pycache__' not in p.parts)
        self.implementation_hash=digest(b''.join(f.name.encode()+b'\0'+f.read_bytes()+b'\0' for f in files))
        self.session=digest(stable_json({'source':digest(source),'symbol':symbol,
           'contract':self.contract,'visible':sorted(selected),'implementation':self.implementation_hash}))
    ...
    def packet(self, site: str | None = None) -> dict[str,Any]:
        ...
        for prefix,base,lo,hi in self.parsed.families:
            if base in origins:context.append({'family':prefix,'source':f'family {prefix} = {base}[{lo}..{hi}];'})
        ...
        p={'protocol':'cairn.packet/1','session':self.session,'symbol':self.symbol,
           'signature':signature(self.f),'profile':VERSION,
           ...
           'source_sha256':digest(self.source),'implementation_sha256':self.implementation_hash,
        ...
        if site is not None:
            if site not in self.sites:fail('E-SITE','Unknown expression site.')
            focus=self.sites[site]
            p['focus']=focus
            p['draft_protocol']={'protocol':PROTOCOL,'session':self.session,'kind':'expr','site':site,'replacement':focus['source']}
        return p

    def check(self, request: dict[str,Any]) -> tuple[str,dict[str,Any]]:
        if not isinstance(request,dict):fail('E-REQUEST','Edit request must be an object.')
        kind=request.get('kind')
        keys={'protocol','session','kind','replacement'} | ({'site'} if kind=='expr' else set())
        if set(request)!=keys:fail('E-REQUEST','Missing or unknown edit fields.', fields=sorted(set(request)^keys))
        if request.get('protocol')!=PROTOCOL:fail('E-REQUEST','Unsupported edit protocol.')
        if request.get('session')!=self.session:fail('E-SESSION','Stale or mismatched source, policy, context, or toolchain.')
        ...
        if kind=='body':
            parser=Parser(text)
            if parser.eat('='):
                if self.f.ret.name == 'void': fail('E-EXPRESSION-BODY', 'Expression bodies cannot return void.')
                parser.expr(); parser.need(';')
            else:
                parser.block()
            parser.need('<eof>')
            ...
            if not isinstance(key,str) or key not in self.sites:fail('E-SITE','Unknown or stale expression site.')
            parser=Parser(text); parser.expr(); parser.need('<eof>')
            ...
            text='('+text+')'
        else:fail('E-REQUEST','Expected body or expr edit kind.')
        ...
        if signature(f)!=signature(self.f):fail('E-SIGNATURE','Function signature changed.')
        if set(receipt['functions'])!=set(self.receipt['functions']):fail('E-DECLARATION','Declaration set changed.')
        additions=set(receipt['functions'][self.symbol]['effects'])-self.allowed_effects
        if additions:fail('E-EFFECT-EXPANSION','Candidate exceeds its effect ceiling.', added_effects=sorted(additions))
        unknown=set(receipt['functions'][self.symbol]['calls'])-self.visible
        if unknown:fail('E-CONTEXT-CLOSURE','Candidate introduces an undisclosed callee.', symbols=sorted(unknown))
        for name,r in receipt['functions'].items():
            if name!=self.symbol:
                delta=set(r['effects'])-set(self.receipt['functions'][name]['effects'])
                if delta:fail('E-CALLER-EFFECT','Candidate expands an unchanged caller footprint.', symbol=name, added_effects=sorted(delta))
        return candidate,{'protocol':'cairn.admission/1','status':'typed',
            'session':self.session,'symbol':self.symbol,'candidate_sha256':digest(candidate),
            ...
            'source_outside_edit_unchanged':True,'contract_unchanged':True,
            'full_module_rechecked':True,'native_build':'not-run','behavioral_tests':'not-run',
            'equivalence':'not-proved','formal_status':'not-verified'}

    def candidates(self, site: str, expressions: list[str]) -> list[dict[str,Any]]:
        if len(expressions)>128:fail('E-REQUEST','At most 128 candidates per query.')
        results=[]

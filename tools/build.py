...
def main():
    ...
    if not cxx: raise SystemExit(f'Compiler not found: {a.cxx}. Install Clang or GCC; no files downloaded.')
    source=a.source.resolve()
    try: generated,receipt=compile_source(source.read_text())
    except Diagnostic as e:
        print(json.dumps(e.data),file=sys.stderr); return 1
    a.out.mkdir(parents=True,exist_ok=True)
    cpp=a.out/(source.stem+'.cpp'); cpp.write_text(generated)
    (a.out/'cairn_runtime.hpp').write_text(RUNTIME)
    lib=a.out/('lib'+source.stem+'.so')
    flags=['-std=c++20','-O3',f'-march={a.arch}','-ffp-contract=off','-fno-fast-math',
    ...
    receipt['native_build']={'command':command,'exit_code':result.returncode,

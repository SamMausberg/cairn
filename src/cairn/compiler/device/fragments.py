...
operation, legal only where each warp of a cooperative block reaches it whole (compiler/cooperative/cooperative.py, E-COOP-WARP),
and nowhere outside one (E-FRAGMENT). The tile is a shared array of the block, or a device view, and the layout the
operation names says where each element of it lives (compiler/device/layouts.py). Each family reads what it can:
...
in every thread of a warp (E-COOP-WARP, from compiler/cooperative/cooperative.py's `Reach`); an mma.sync accumulator may differ,
...
from ..syntax.tree import USIZE, VOID, Expr, Type, fail, is_view, root
...
def warp(c: Checker, node: Any, what: str):
    ...
    cooperative = import_module("..cooperative.cooperative", __package__)  # the cooperative layer, which sets c.coop
...
def uniform(c: Checker, args: list[Expr], what: str):
    ...
    cooperative = import_module("..cooperative.cooperative", __package__)
    for a in args:
...
def footprint(c: Checker, e: Expr, i: int, j: int) -> list[tuple[int, int | None]]:
    ...
    (compiler/cooperative/block_run.py)."""
...
def check_mma(c: Checker, e: Expr, args: list[Expr]) -> Type:
    ...
    from ..primitives.builtins import contract

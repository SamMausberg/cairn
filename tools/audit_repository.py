...
BAD_NAMES = {'.env', '.netrc', '.npmrc', '.pypirc', 'credentials.json',
             'id_rsa', 'id_ed25519', 'hosts.yml'}
...
def audit(root: Path) -> dict:
    ...
    for commit in commits:
        for record in git('ls-tree', '-r', '-z', commit).split(b'\0'):
            ...
            if mode not in {'100644', '100755'} or kind != 'blob':
                findings.append({'path': name, 'rule': 'symlink-or-submodule', 'commit': commit}); continue
            if oid in seen:
                continue
            seen.add(oid)
            if len(seen) > 20000:
            ...
            if size > 2_000_000:
                findings.append({'path': name, 'rule': 'large-file', 'commit': commit}); continue
            data = git('cat-file', 'blob', oid); count += len(data)
            if b'\0' in data:
    ...
    return {'status': 'no-pattern-findings' if not findings else 'blocked',
            'commits': len(commits), 'distinct_blobs': len(seen), 'bytes_scanned': count,
            'findings': findings, 'scope': 'all reachable commits; finite credential patterns, not a guarantee'}


def main() -> int:
    p = argparse.ArgumentParser(description=__doc__); p.add_argument('root', nargs='?', type=Path, default=Path(__file__).resolve().parents[1])
    a=p.parse_args(); result=audit(a.root);print(json.dumps(result,indent=2))

...
| a Buf of records | 6 (4) | 5,844 | 1,354 | 2,663 (`grep -rn "Buf" docs/*.md`) | `--takes usize --returns 'Buf[Rec]'` | 43 / 34 |
| tasks and groups | 9 (8) | 49,914 | 8,340 | 8,340 (`docs/concurrency.md`) | `spawn task group` | 69 / 60 |
| the limits of i64 | 5 (1) | 1,725 | 1,725 | 726 (`grep` of `docs/roadmap.md`) | `i64 minimum` | 100 / 91 |
...
| | | | | | `--takes 'ro<u8>[n]' --takes 'ro<u8>[n]' --returns usize` | 401 / 391 |
| the library, whole | 33 (10) | 55,570 | 6,834 | 8,623 (`docs/library.md`) | none | |

Every answer is smaller than the median subject's reading for its question: 11 to 121 times smaller for a word query, and 31 times for the `Buf` type query. A type query that many functions fit answers with its ten best lines and a count of the rest, 344 and 401 tokens here, about a third of what the median subject read for the same fact. The whole-library reads, `library.md`, `std_api.md` and `cairn doc --std` read up front, have no single query that replaces them: each question above is one query.

What each answer names, from `sizes.json`: `std.io.read_stdin`; `std.text.parse_i64` and `std.text.parse_u64`; `std.vec.push` and `std.vec.reserve`, or for `Vec[Rec]` seventeen, best fit first: twelve of `std.vec`, `std.mem.copy` and `fill`, `std.sort.sort_by`, `std.arena.insert` and the builtin `take`; `Buf`; the builtins `Group` and `wait`; `std.text.write_i64` and the conversion line that states the literal `-9223372036854775808`; `add_wrap`, whose line says that checked `+ - *` abort on overflow, signed or unsigned; `std.text.find`, or for two views and a `usize` six functions headed by `std.sort.search` and `std.text.find`.

A query answered from words took about a millisecond once the index was built, and building it for the library took 0.26 seconds. A type query took 0.54 seconds for one view, 0.61 for a `Vec[i64]` and 1.26 for two views, the median of three, because it checks one probe for every way the values fill every function's parameters, up to about 2,000 probes. The load average was between 50 and 120 while these timings ran, from other agents' test suites.
